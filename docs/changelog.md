...
**新增了一条龙功能，一键完成日常，更多一条龙功能计���请查看 https://github.com/babalae/better-genshin-impact/issues/846**
...
* 联机情况下识别角色队伍时，修复错误识别房���的问题（会导致队伍识别出错） #777
...
* 残血、死亡时自动传送至七天神像
* 恢复界面上单独执行任务的右键菜单
* 添加路线重试机制、卡死脱离3次后放弃当前路径进入重试、卡死脱离��加超时时间
...
* TP时，当前地图位置识别失败的情况下，会直接通过国家位置进��大地图定位
* TP失败时加入重试机制
...
* 修复伐木上限配置无法突破2000的问题（荒泷一斗被动可以突破���个值）。 #606
...
> [!IMPORTANT]
> **地图追踪功能**：自动传送到最近的锚点，然后根据���地图开始寻路。系统会根据任务文件的途径点和目标点进行导航。
...
## 优化与改动
* 修复自动邀约班尼特、诺艾��第二幕无法正确选择结局的问题 #581
...
- 修复���新提示弹窗位置未适配高分屏DPI缩放 #446 @emako 
...
* 自���伐木默认不再开启OCR识别木材数量的功能（部分木材100%识别失败） #444
* 自动剧情添加了随机点击选项的选择
