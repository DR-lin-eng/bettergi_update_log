...
- 优化每日委托���奖，使用点击屏幕替代按ESC (#2761) @zaodonganqi
...
- **地图特征使用二进制文件存��，减小文件体积，大幅提升特征数据加载速度** @shatyuka
...
- 自动剧情关闭弹出页前判断是否是大地图的弹出��面，以防误识别 #1466
...
* **后台自动剧情模式不��会瞬移鼠标点击，直接使用5.2的F**，注意对话结束后鼠标仍会被吸附到游戏窗口内！
