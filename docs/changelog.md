...
* 修复更新窗口色差以及白边的问题
* 修复暂停���恢复异常bug，调度器设置增加周期配置，执行调度器任务增加继续执行功能 @mfkvfhpdx
...
- 修复创建配置组添加js脚本后，首次编辑js脚本自��义配置时，组件无法正确绑定到setting.json中的默认值 (#1449) @breadgrocery
- 调用切换队伍方法后，清除队伍缓存
...
- **新增支持独立地图层岩巨渊、渊下宫，录制编辑器和追踪��图查看同步支持**
...
```
* **修复全自动钓鱼导致机器内存占���的问题** @FishmanTheMurloc 
...
* 自动秘境功能存在角色死亡后，如果设置了目标秘境，则��自动传送回去继续自动秘境
...
**添加了脚本仓库，在线版 [https://bgi.sh/](https://bgi.sh/) （感谢 @DR-lin-eng 友情提供的域名），BetterGI本��也内嵌了相同的网页。**
...
只支持最大的地表���图，不支持地底识别，也不支持渊下宫、层岩巨渊地底地图。由于自带了原神地图的全特征点文件，软件体积暴增。
...
* 自动剧情文本描述更新 #588
* 自动伐木：首次伐木就未识别��木材数据，会自动关闭OCR识别并累计木材数的功能
```
...
- 键鼠脚本的录制与回放从相对时间修改为绝对时间，大幅提高精度，减少卡顿，**但是不兼容 0.33 版���前的键鼠脚本** @lwh9346 
...
* 七圣召唤新增新的卡组策略 `9.芙芙卡维鲸鱼.txt`，使用此���略的时候手牌请塞满3费卡，感谢群友 `繁星入梦` 编写的策略
...
如果你���时使用BitBlt截图模式无法正常使用BetterGI，且自动活动音游功能也无法使用，可以尝试此方法： https://github.com/babalae/better-genshin-impact/issues/92
...
## 快捷键
- 快捷键新增键鼠监听模式，**支��任意键盘单键、鼠标侧键（XButton1、XButton2）**，不支持组合键，在更好的原神启动后才生效。#6  #45 #123 #228
...
- **新增了自动提交物品的功能**。由于没有找到多��物品的测试场景，所以现在只支持一个物品。
...
- WindowsGraphicsCapture截图方式的优化，确保能够获取到最新图片，减少截图失��的报错
