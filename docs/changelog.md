...
```
近期为了支持后续功能扩展，���解决历史遗留问题，底层能力正在进行重构，版本BUG较多，请谨慎选择更新！
...
- 一条龙结束后可推送原粹树脂、浓缩树脂和每日���托奖励汇总及截图，识别失败时降级为部分结果 (#3540) @antecanis8
...
- 修复背��数量 OCR 首尾空白和物品图标后台保存时的对象释放问题 (#3532) @taozhe6
...
- 脚本配置组改用原子写入，降低保存中断导致配���损坏的风险 (#3502) @physligl
...
- 统一页面排版、边距和滚动交互，并修复主导航容器显示��节 (#3303) @Arkadegong
...
- 添加通过 OCR 识别当前���色 UID 的 JS API (#2988) @Kirito520Asuna
...
- **添加进出千星以刷新木��CD的能力** #2677 #2682 @zaodonganqi
- 七圣召唤不再等待吞星之鲸加血 (#2679)
...
## 其他
- **同时启动原神的功能，添加使用CMD启动游戏的入口，如果你出现“检测到非法工具，请重启���器”的提示，可以尝试此功能**
...
- 修复 AutoDomainParam 泛型���义 @breadgrocery
...
- 适配 6.0 菈乌玛退队后队伍UI不会还原，但是跨地图��送后UI还原 的场景
...
- 提高SetTimeTask的稳定�� (#2161) @JamisHoo
...
- 地图追踪版本要求修改为强制
- 优化地图追踪的距离过远判断
- 修复地图追踪���角色公转的场景，持续角度一直未在5度内时，直接强制等到转向到目标位置 #1644
...
* 软件自带��认黑名单与用户配置黑名单拆分，不会再出现用户配置的黑名单被覆盖的情况了
* 配置黑白名单可实时生效
* 修复自动拾取展示文本可能缺失标点的问题
...
![image](https://github.com/user-attachments/assets/47b54ac1-b524-4f62-a7a0-8e2b83221cf0)
    ...
    * 解决方案：关掉后重试一次，手动运行BetterGl.update.exe（此时打开软件���该已经是最新版了）。如果还是不行，可以直接重新下载安装包安装。
...
- 修复���DPI下TP地图往复移动的问题 (#1469) @shatyuka
...
* 自动战斗后、传送前都会释放所有���键，尝试解决TP神像后持续往前走的问题
* 修复火元素采集不生效的问题
* 优化自动传送的稳定性 @physligl
