...
```
6.6 版本适配（新角色、空之神殿地图、传送点、七圣召唤），莉奈娅挖矿改用 YOLO 模型等
...
### JS脚本
- 为 JS 脚本提供 HTML 遮罩功能，支持点击穿透��式切换 (#2975 #3100) @zaodonganqi @1004452714
...
- 修复模糊黑名���读取错误的文件路径 (#2635) @yanmofeixi
- 一条龙冒险家协会添加挪德卡莱
...
- 退回到主界面函数支持识别更多场景
- 暴露设置时间方法（跳过���画实测未生效） (#2426) @zaodonganqi
...
- 支持自定义脚本仓库地址 @ddaodan
- 支持在脚本中进行 HTTP 请求，并实现 URL ���别细粒度控制和 UI 提示  (#2331)
...
- 地图追踪版本要求修改为强制
- 优化地图追踪的距离过远判断
- 修复地图��踪中角色公转的场景，持续角度一直未在5度内时，直接强制等到转向到目标位置 #1644
...
* JS 脚本新增文件写入、输入文字的能力
* 修复 关���截图器，切换分辨率，开启截图器后遮罩窗口位置不正常 的问题
...
* ��复地图追踪中的 up_down_grab_leaf 四叶印 action 不生效的问题
...
* 自动秘境中的战斗策略存在火神的情况下，战斗结束��自动切人 #1052
```
