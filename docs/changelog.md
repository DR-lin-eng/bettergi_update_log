...
- 优化每日委托领���，使用点击屏幕替代按ESC (#2761) @zaodonganqi
...
- **文字菜单UI区域适配，修复自动领取尘歌壶在新版本报错的��题** (#2511) @kaedelcb
- 每日奖励领取适配
...
- 删除这两个选项：是否就近七天神像恢复血量、传送���七天神像之后是否需要移动后回血
...
* 优化BitBlt截图��辑，增加失败重试机制以提高截图成功率 (#1407) @Takaranoao
...
```
> [!CAUTION]
> 由于本次更新修改了底层截图器与任务调度，这并不是一个稳定的��本，请谨慎选择更新！
...
* 优化一条龙领取奖励时，橙色选项的识别率
* 修改所有自动战斗的键鼠模拟方式，尝试修复自���战斗原地不动的问题
* 部分UI改造，部分文案修正
```
...
* 添加 `log_output` 的 action 用于��指定点输出日志
...
* 调度器添加���图路径追踪任务时候的ui优化，默认不展开，且添加筛选功能
...
### 调度器
* 添加配置组设置功能，可以配置路径追踪队伍切换、使��盾、回血角色等
* 添加连续执行的功能
...
由 @HZYgrandma 贡献，由于使用的是和自动晶蝶一样的自动传送+键鼠录制的方式，长距离可能无法精准到达点位。且由于键鼠录制不够稳定，出现无法正确拾取狗粮的���况属于正常情况。后续可能会通过地图追踪功能重写。
...
- 启动时检查是否有关键文件��失，缺失时提示 #468 @qhy040404 
...
* 七圣召唤新增新的卡组策略 `9.芙芙卡维鲸鱼.txt`，使用此策略的时候手牌请塞满3费卡，���谢群友 `繁星入梦` 编写的策略
