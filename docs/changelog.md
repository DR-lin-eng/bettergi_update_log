...
- 修复高缩放��日志子窗口消失 (#2585) @ShadowLemoon
...
- 删除 “允许在jsScript脚本中使用此地图追踪配置” 和 “允许在此调度器中（一般在JS脚本中）调用自动战斗任务时，采用此追踪��置里的战斗策略” 配置。默认启用
- 脚本仓库更新添加进度展示
...
## 地图追踪
* 通过点位配置，支持在未识别点位情况下，��大地图中心点来识别坐标。 #1526 @mfkvfhpdx
* 挖矿策略修改为只执行一个角色
...
* 进入对话中时会自动选择选项并退出。
* 修复路径追踪部分变量写死的问题
* 小于 1920x1080 的���戏分辨率使用路径追踪会触发提示
...
* **队伍识别支持联机情况下的识别**
* 重新训练了头像分类器，使其支持角色头像处于半透明情况下的识别（攀爬、跳跃的时候右侧头像会变半透明，切人带数字的情况仍���支持）
* 修复后台自动剧情启用的情况下，自动领取探索奖励失效的问题
* 界面UI更新
