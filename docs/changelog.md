...
- 修复键鼠文件夹不存在的问题 #2316
- **适配6.0版本后 40、20 树脂���计算，自动秘境不会再多刷一次了**
...
- 切换队伍适配新版本最大队伍数由15增加到20，并新增最后一个非空���伍判断 @FishmanTheMurloc
...
- 飞书通知时带有图片(需要配置app,并且在飞书后��增加上传图片权限) @niuhuan
...
* **后台自动剧情模式不再会瞬移鼠标点击，直接使用5.2的F**，注意对话结束后鼠标仍会被��附到游戏窗口内！
