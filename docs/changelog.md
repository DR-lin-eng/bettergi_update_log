...
- 检��战斗结束增加旋转寻找敌人实验性功能 (#1960) @kaedelcb
...
- 降低飞行检测时间，自由落体后进入飞���再下落攻击 (#1837) @MisakaAldrich
...
* 软件自带默认黑名单与用户配置黑名单拆分，不会再出现用户配置���黑名单被覆盖的情况了
* 配置黑白名单可实时生效
* 修复自动拾取展示文本可能缺失标点的问题
...
* TP时，当前地图位置识��失败的情况下，会直接通过国家位置进行大地图定位
* TP失败时加入重试机制
...
* 修复伐木上限配置无��突破2000的问题（荒泷一斗被动可以突破这个值）。 #606
...
## 自动伐木
* OCR识别并统计伐木数量，加入限制伐木��量配置 @atiasn
