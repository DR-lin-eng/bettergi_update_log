...
### 地图追踪
- 使用 YOLO 进行莉奈娅挖矿，优化配置与��错 (#3058) @zaodonganqi
...
- 调度器配置中增加执行成功任务���过执行配置 (#1893) @mfkvfhpdx
...
![image](https://github.com/user-attachments/assets/47b54ac1-b524-4f62-a7a0-8e2b83221cf0)
    ...
    * 解决方案：关掉后重试一次，手动运行BetterGl.update.exe（此时打开软件应该已经是最新版了）。如果还是不行，可以直接重新下载安装包安���。
...
* 截图器优化。 #1480 #1573 @shatyuka 
    ...
    * WGC 优化窗口大小检测优化
    * “选择捕获窗口”优化，置顶原神等
* 使用 Image<Rgb24>替换 Bitmap，减少 YOLO 推理时图像��式转换带来的损失 #1563 @shatyuka
...
```
> [!CAUTION]
> 由于本次更新修改了底层截图器与任务��度，这并不是一个稳定的版本，请谨慎选择更新！
...
* 地图追踪中异常场景处理��加 遇到 x 按钮时候 按下ESC返回主界面
