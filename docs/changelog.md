...
- 修复切换队���拾取初始队伍为空的异常识别 (#2688) @kaedelcb
...
## 自动秘境/幽境/���斗
- 战斗策略中，��一个命令和当前命令不是同一个角色时，直接切换角色 #2411
...
- 优化自动剧情-可提交物品的识���方法 (#2477) @1004452714
...
- 删除这两个选项：是否就近七天神��恢复血量、传送到七天神像之后是否需要移动后回血
...
## 底层改造
* OCR平台改造。移除 PaddleSharp ，使用 ONNX 直接进行推理，���持更多推理环境 #1459 @Takaranoao 
...
- 发送通知时，添加延迟���图片先发送，时间戳与事件之间添加换行 @MisakaAldrich
...
## 调度器
* 调度器清空所有任务添加询问提示
* 新增shell类型的��务可供添加（**请不要执行看不懂的命令**）#1248 @Takaranoao
...
* 添加 `log_output` ��� action 用于在指定点输出日志
...
**在自动音游的基础上，新增自动专辑功能。���专辑界面下运行可以自动演奏完所有未领取完奖励的乐曲。**
...
* **后台自动剧情模式不再会瞬移鼠标点击，直接使用5.2的F**，注意对话结束后鼠标仍会被吸附���游戏窗口内！
