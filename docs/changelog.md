...
## 底层改造
* OCR平台改造。移除 PaddleSharp ，使用 ONNX 直接��行推理，支持更多推理环境 #1459 @Takaranoao 
...
- **地图特征使用二进制文件存储，减小文件体积，大幅提升特征数据��载速度** @shatyuka
...
```
* 修复手动设置cd不生效和快速跳��战斗时有时会导致刷屏的问题(#1410) @Takaranoao
...
```
> [!CAUTION]
> 由���本次更新修改了底层截图器与任务调度，这并不是一个稳定的版本，请谨慎选择更新！
