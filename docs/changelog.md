...
- JS脚本中 KeyPress 语法针对功能键也采取��接触发的方式 #2071
...
- 修复迷烟主成���非飞行状态下四叶印识别失败的Bug，模板匹配的置信度修改为数组 @Limint
...
- 优化自动重���登录针对B服的登录框的操作，支持5.8版本的新登录框 @zaodonganqi
...
* 合成浓缩树脂：添加合成浓缩后保留原粹树脂数量的功��� @kaedelcb
...
- 识别选择队伍后队伍切换成功的���景，减少因为切换时间过长而引发的队伍切换失败的问题 #1457
...
* 在未识别到配队选择页面时重试
* 新增命令行： `.\BetterGI.exe startOneDragon`，支���在软件启动后直接启动一条龙 #998
...
> [!IMPORTANT]
> **非常感谢 @Limint 贡献的新的相机视角识别算法，解决了路径���踪时视角晃动的问题**
...
> [!IMPORTANT]
> **地图追踪功能**：自动传送到最近的锚点，然后根据小地图开始寻路。系统会根据任务文件的途径���和目标点进行导航。
...
* **WindowsGraphicsCapture 在 Windows 10 version 1903 以下版本中（不含）不再展���（因为系统不支持）** #394
...
- 自动秘境指定轮数完成后，退出副本
- 所有独立任务都新增不可重入的互斥锁，同一时间只能存在���个独立任务正在运行 #304
- 自动伐木内存回收优化 #311
