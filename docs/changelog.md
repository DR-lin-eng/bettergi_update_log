...
- 适配月绽放角色（菈乌玛）入队后，角色无法正常切换的问题。（右侧队伍下方出现草露进度条，会让整个队伍UI上移，后续换队后UI依旧保���原样）
...
- 修改部分角色挖��的方式 @RRRR623 @MisakaAldrich
...
## 自动拾取
* 优化文字识别功能，使用新的文字区域检测方式减少CPU消耗（**不会出现 100% CPU的情���了**）
...
- 将���动秘境战斗失败复活次数从常量3修改为可更改的参数 @xiaokang00010
- 日志输出优化
...
* 支持在命令行启动一条龙参数���选择配置，配置找不到就用界面指定的 #1586 @zzc-tongji
...
- 修复创建配置组添加js脚本后，首次编辑js脚本自定义配置��，组件无法正确绑定到setting.json中的默认值 (#1449) @breadgrocery
- 调用切换队伍方法后，清除队伍缓存
...
- **新增支持独立地图层岩巨渊、渊下宫，录制编辑器和追踪地图查看��步支持**
...
* 添加使用Starward记录游戏时间的��能 @kndxhz #1164
...
* 修复windows10 1909版本使用WGC会报错��问题 @masm611 #954
* 更多傻瓜化日志提示
...
**新增了一条龙功能，一键完成日常，更多一条龙功���计划请查看 https://github.com/babalae/better-genshin-impact/issues/846**
...
* 联机情况下识别角色队��时，修复错误识别房主的问题（会导致队伍识别出错） #777
...
* 修复伐木上限配置无法突破2000的问题（��泷一斗被动可以突破这个值）。 #606
...
只支持最大的地表地图，不支持地底识别，也不支持渊下宫、层岩巨渊地底地图。由于自带了���神地图的全特征点文件，软件体积暴增。
...
**感觉自动拾取有掉帧的朋友可以切换到GPU进行推理，然后自动拾取文字识别引擎选�� Yap，这样 CPU 占用会大幅降低。**
...
**此改��过大，未完整进行测试，如果出现部分功能不可用的情况，请提 Issue 告诉我。**
...
为了解决自动拾取的高占用，新增了一个OCR识别引��，识别速度稍慢但是占有低，并作为默认选项。解决此类卡顿问题：#383
...
> * ���斗脚本使用角色官方名称 **“阿蕾奇诺”无法正常解析** 的问题，可以使用别名“仆人”替代，或者手动覆盖相关配置文件
>
...
* 角色识别模型加入`仆人`的识别（用于一键宏、自动战斗/秘境功能中的队伍���别）
...
## 其他优化
* Yap 自动拾取模型更新
* 修复原神启动时，`config.ini` 不生效的问题（用了官B��换工具，但使用BetterGI启动时永远是官服）
