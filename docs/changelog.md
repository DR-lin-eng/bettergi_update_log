...
- 提供自���义参数的 自动秘境 与 自动战斗 方法 (#2299) @1004452714
...
- 自动秘境换队从在门口切换���改为在进门后切换；原粹识别优化 (#1866) @kaedelcb
...
- 内嵌 VC++ 运行库 14.44.35208.0，**��试彻底解决“Unable to load DLL 'onnxruntime'or one of its dependencies: 动态链接库(DLL)初始化例程失败” 的问题**
...
- 新增关于页
- 迁移按键绑定页面到设置中
- 自动钓鱼恢复 16:9 的支持，**与一些钓鱼功能��优化** @FishmanTheMurloc 
- 一些UI界面上的变化
...
* 残血、死亡时自动传送至七天神像
* 恢复界面上单独执行任务的右键菜单
* 添加路线重试机制、卡死脱离3次后放弃当前路径���入重试、卡死脱离添加超时时间
...
* TP时，当前地图位置识别失败的情况下，会���接通过国家位置进行大地图定位
* TP失败时加入重试机制
