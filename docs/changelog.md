...
- 调度器配置中增加执行���功任务跳过执行配置 (#1893) @mfkvfhpdx
...
* 自动秘境中的战斗策略存在火神的情况下，战斗结��后自动切人 #1052
```
...
1. **建议在游戏内配合快捷键使用**。
2. 录制时遇到主界面和其他���面的切换时，不要操作，需要等待程序识别这个切换过程
