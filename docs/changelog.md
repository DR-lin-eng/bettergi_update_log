...
```
本次适配原神7.0至冬内容，���增自动演奏和自动拾取白名单，优化地图传送稳定性。
...
- 优化奖励页就绪检测与数���识别，减少奖励汇总遗漏 (#3476 #3494) @1004452714
...
- 新增角色战技赶路，支持���名角色与连续赶路模式，并完善体力判断和下车逻辑 (#3280 #3319 #3328 #3336 #3367) @Bedrockx
...
- 一条龙支持重复添加任务和从指定书签继续执行，并保留���任务实例的独立配置 (#3276) @Bedrockx
...
- 优化任务状态机的���场等待与当前节点重试判定，避免中间态超时被误计为当前节点重试 (#3182) @ShadowLemoon
...
## 其他
- **修复JSON序列化问题，本���原因是OCR模块指针指向的内存出现错误，感谢群友 焦糖玛奇朵
 提供环境协助定位！**。
...
当前只支持提瓦特区域地表地图，不支持分层与独立���图。仅支持16:9的游戏分辨率
...
- 适配更多 6.0 队伍相���改造 @kaedelcb @FishmanTheMurloc
...
- 添加火元素采集选项到条件定义 #2159
- 优化 地图��踪任务添加窗口 性能，修复了索引时文件树复选框不被记忆的BUG (#2157) @Datouyu666
...
- 修复自定义的交互按键未对自动秘境生���的问题 (#2035) @Patrick-Ze
- 地图追踪中新增使用小道具的action
...
- 调度器添加地图追踪文���时自动聚焦搜索框 (#1916) @Arkadegong
...
- 修复地图追踪配置中获取材料名称��问题 (#1919) @kaedelcb
- 地图追踪版本要求修改为强制
- 优化地图追踪的距离过远判断
...
🌩️ [Github 下载太慢？ 点我访问更多更��渠道](https://bettergi.com/download.html)
...
* 支持在命令行启动一条龙参数时选择配置，配置找不到就用界面���定的 #1586 @zzc-tongji
...
## 其他优化
- ���化新版本更新日志窗口的背景显示(未完全解决) (#1413) @richex-cn
...
```
> [!CAUTION]
> 由于本次更新修改了底层截图器与任务调度，这并不��一个稳定的版本，请谨慎选择更新！
...
* 地图追踪中异常场景处理添加 遇到 x 按���时候 按下ESC返回主界面
