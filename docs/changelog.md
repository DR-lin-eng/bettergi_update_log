...
- 优化任务状态机的转场等待与当前��点重试判定，避免中间态超时被误计为当前节点重试 (#3182) @ShadowLemoon
...
- 地图遮罩实时任务内匹配操作异步化，不再阻塞自动拾��等其他触发器
- 修复UI线程死锁的问题
...
- 清理战斗策���，更新万能策略 @MisakaAldrich (#2606)
- 优化进入幽境危战的流程 (#2537)
- 彻底解决出战角色识别不正常的问题
...
- **文字菜单UI区域适配，修复���动领取尘歌壶在新版本报错的问题** (#2511) @kaedelcb
- 每日奖励领取适配
...
- 修复万叶长E拾取判断���CD计算错误 (#1958) @kaedelcb
