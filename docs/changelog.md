...
- 添加火元素采集选项到条件定义 #2159
- 优化 ���图追踪任务添加窗口 性能，修复了索引时文件树复选框不被记忆的BUG (#2157) @Datouyu666
...
- 取消兑换码使用后不再弹出此剪切板内容的兑换码，新增兑换码剪切板监听的开关���置
...
- 修复配置组名称前半段有同名不显示���问题 (#1858) @kaedelcb
...
![image](https://github.com/user-attachments/assets/47b54ac1-b524-4f62-a7a0-8e2b83221cf0)
    ...
    * ���决方案：关掉后重试一次，手动运行BetterGl.update.exe（此时打开软件应该已经是最新版了）。如果还是不行，可以直接重新下载安装包安装。
...
## 自动拾取
- **拾取列表已满时候，滑动滚轮（注意这个变更会会导致拾取变���从下往上拾取，对于尘歌壶收菜有一定影响）**
...
* 自动战斗后、传送前都会释放所有按键，尝试解决TP神像后持续往前���的问题
* 修复火元素采集不生效的问题
* 优化自动传送的稳定性 @physligl
...
* 修复伐木上限配置无法突破2000的问题（荒泷��斗被动可以突破这个值）。 #606
...
* 自动剧情文本描述更新 #588
* ���动伐木：首次伐木就未识别到木材数据，会自动关闭OCR识别并累计木材数的功能
```
...
* 新增一种**后台点击模式**，开启时自动剧情和邀约可以在后台被点击。
	* **但是请注意游戏不能最小化，切在后台选择选项时候鼠标仍旧会移动！自动剧情结束后鼠标会被吸附回游戏中，请按<kbd>Alt</kbd>+<kbd>Tab</kbd>键切回���游戏**
...
如果你平时使用BitBlt截图模式无法正常使用BetterGI，且自动活动音游功能也无法使���，可以尝试此方法： https://github.com/babalae/better-genshin-impact/issues/92
