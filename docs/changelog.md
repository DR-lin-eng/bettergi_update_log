...
- ��遗物分解套装筛选的改进和JS引擎的约束 (#2319) @FishmanTheMurloc
...
- 添加火元素采集选项到条件定义 #2159
- 优化 地图追踪任务添加���口 性能，修复了索引时文件树复选框不被记忆的BUG (#2157) @Datouyu666
...
- 内嵌 VC++ 运行库 14.44.35208.0，**尝试彻底解决“Unable to load DLL 'onnxruntime'or one of its dependencies: 动态��接库(DLL)初始化例程失败” 的问题**
...
![image](https://github.com/user-attachments/assets/47b54ac1-b524-4f62-a7a0-8e2b83221cf0)
    ...
    * 解决方案：关掉后重试一次，手动运行BetterGl.update.exe（此时��开软件应该已经是最新版了）。如果还是不行，可以直接重新下载安装包安装。
...
* JS 脚本新增文件写入、输入文字的能力
* 修�� 关闭截图器，切换分辨率，开启截图器后遮罩窗口位置不正常 的问题
...
* 新增路径点位 `orientation` ��于控制人物朝向 @Yang-z 
* 路径追踪中自动跳过剧情的配置为强制跳过
...
* 自动秘境中的战斗策略存在火神的情况下，战斗��束后自动切人 #1052
```
...
* 残血、死亡时自动传送至七天神像
* 恢复界面上单独执行任务的右键菜单
* 添加路线重试机制、卡死脱离3���后放弃当前路径进入重试、卡死脱离添加超时时间
...
* TP时，当前地图位置识别���败的情况下，会直接通过国家位置进行大地图定位
* TP失败时加入重试机制
...
* 修复伐木上限配置无法���破2000的问题（荒泷一斗被动可以突破这个值）。 #606
...
> [!IMPORTANT]
> **地图追踪功��**：自动传送到最近的锚点，然后根据小地图开始寻路。系统会根据任务文件的途径点和目标点进行导航。
...
1. **建议在游戏内配合快捷键使用**。
2. 录制时遇到主界面和其��界面的切换时，不要操作，需要等待程序识别这个切换过程
...
## 自动伐木
* OCR识别并统计伐木数量，加入限制伐木总���配置 @atiasn
