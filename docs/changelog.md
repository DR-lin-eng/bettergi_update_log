...
- 修复并发调用小地图识别时识别���误的Bug，修改并增加暴露给JS的小地图识别函数 (#1911) @Limint
...
- 将自动秘境战斗失败复活次数从常量3修改为可更改的���数 @xiaokang00010
- 日志输出优化
...
* 将路径追踪相关���日志改为debug级别 (#1645) (#1684) @Patrick-Ze
...
由于键鼠脚本分享时，���同电脑存在很多影响变量，**测试发现存在大量无法正确重放的情况**。
...
#### 如果在游戏大���为2K、4K且截图方式为`WindowsGraphicsCapture`的情况下，出现卡顿、鼠标移动缓慢等问题， 参考：https://github.com/babalae/better-genshin-impact/issues/286 关闭位图缓存。或者直接切换截图方式为`BitBlt`，Win11 `BitBlt`不能用？参考设置：https://github.com/babalae/better-genshin-impact/issues/92
...
* 队伍识别失败时，沿用上一次识别��功的队伍。（算是解决了切换角色 1s CD 的时候，由于头像虚化导致队伍无法识别的问题）
* 配置修改后立即生效
...
- 快捷键新增键鼠监听模式，**支持任意键盘单键、鼠标侧键（XButton1、XButton2）**，不支持组合键，在更好的原神启动后才生效。#6  #45 #123 #228
    ...
    - 键鼠监听：支持任意键盘单键、鼠标侧键，功能启动后才生效（推荐）。注意鼠标的标准是只有两个侧键的，���余侧键请自行用鼠标驱动映射按键。不建议继续在此种方式下使用F开头的功能键，会和原神的快捷键冲突。不支持组合键。
