...
- 一条龙结束后可推送原粹树脂、浓缩树脂和每��委托奖励汇总及截图，识别失败时降级为部分结果 (#3540) @antecanis8
...
### 自动战斗
- 优化战斗结束检测，支持派蒙辅助识别和更灵��的检查时机配置 (#3378) @Bedrockx
- 修复游泳检测提前释放截图对象导致的异常
...
- 优化全局异常处理和启动失败兜��，修复非 UI 线程弹窗造成的内存泄漏 (#3454 #3461) @guamian1337
...
**新增[桌面分身](https://www.bettergi.com/feats/command/session.html)功能，你可以在桌面分身中启�� BetterGI 与游戏，可以做到游戏不占用前台鼠标！注意该功能设置上存在难度，请仔细阅读相关文档。**
...
### 独立任务
- 地脉花新增"蒙德8-风车镇"路径，补充���斗中寻敌配置，修复战后切队与结束检测问题 (#3029 #3059 #3088) @ddaodan
...
- 修改调度器配置默���值，提供新建配置组预设参数 (#3077) @RRRR623
...
- 增加对话区判定次数 #2936 
- 优化TpTask惯性��航（误识别），处理初始未识别 (#2920) @physligl
- 切换角色问题测试用代码 
- 一条龙结束后关闭开书 #2686 
- 最后匹配自定义游戏进程名 #2909 
...
- 地图追踪增加读取目录下的内容等���法 (#2875) @1004452714
...
当前只支���提瓦特区域地表地图，不支持分层与独立地图。仅支持16:9的游戏分辨率
...
- 修复模糊黑��单读取错误的文件路径 (#2635) @yanmofeixi
- 一条龙冒险家协会添加挪德卡莱
...
- 退回到主界面函数支持识别更多场景
- 暴露设置时间方法（跳过动��实测未生效） (#2426) @zaodonganqi
...
- 支持自定义脚本仓库地址 @ddaodan
- 支持在脚本中进行 HTTP 请求，并实现 URL 级��细粒度控制和 UI 提示  (#2331)
...
- 修复自定义的交���按键未对自动秘境生效的问题 (#2035) @Patrick-Ze
- 地图追踪中新增使用小道具的action
...
- ���度器添加地图追踪文件时自动聚焦搜索框 (#1916) @Arkadegong
...
- 修复地图追��配置中获取材料名称的问题 (#1919) @kaedelcb
- 地图追踪版本要求修改为强制
- 优化地图追踪的距离过远判断
...
* 软件自带默���黑名单与用户配置黑名单拆分，不会再出现用户配置的黑名单被覆盖的情况了
* 配置黑白名单可实时生效
* 修复自动拾取展示文本可能缺失标点的问题
...
## 其他优化
- 优化新版本更新日志窗口的背景显示(未完��解决) (#1413) @richex-cn
...
```
> [!CAUTION]
> 由于本次更新修改了底层截图器与任务调度，这并不是一个稳定的版本，请谨慎��择更新！
