...
- 自动地脉���独立战斗配置与OCR遮罩 (#2829) @ddaodan
...
- ���图追踪增加读取目录下的内容等方法 (#2875) @1004452714
...
当前只支持提瓦特区域地表地图，不支持分层与独立地���。仅支持16:9的游戏分辨率
...
- 脚本读取��片时支持调整到指定尺寸 (#2446) @breadgrocery
...
- 配置弹窗UI优化，��新截取物品图标导航地址 (#2499) @zaodonganqi
...
- 支持自定义脚本仓库地址 @ddaodan
- 支持��脚本中进行 HTTP 请求，并实现 URL 级别细粒度控制和 UI 提示  (#2331)
...
- 适配地图传送界面新出现“游逸旅闻：[地区名]”文字，对原有文字��别点击造成干扰的情况 @FishmanTheMurloc
...
- 调度器配置中增加执行成功���务跳过执行配置 (#1893) @mfkvfhpdx
...
* 支持在命令行启动一条龙参数时选择配置，配置找不到就用界面指���的 #1586 @zzc-tongji
...
## 自动拾取
- **拾取列表已满时候，滑动滚轮（注意这个变更会会导致拾取变成从下往上拾取，对于尘歌壶��菜有一定影响）**
...
- ��别选择队伍后队伍切换成功的场景，减少因为切换时间过长而引发的队伍切换失败的问题 #1457
...
- 优化一些报错和日志提示
- 支持5.6新角色的识别
- 修复��个UI上数据绑定的问题 @FishmanTheMurloc
...
* 修复自定义拾取按���不生效的问题 #1262 @Takaranoao
...
* **修复通知导致的截图失败报错提示**
* 使 JS脚本 中的 keyDown(), keyUp(), keyPress() 方法 和 战斗策略脚本 中�� keydown(), keyup(), keypress() 方法 可以直接使用 虚拟键代码 来执行 鼠标操作 #1124 @2696791698
...
* 修复脚本订阅和配置组运行的提示框在高DPI显示屏下跑到屏幕外���问题
