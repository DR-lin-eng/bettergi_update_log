...
```
- **修复一键宏卡死问题** @cookie981
- 优化一条龙每周���境配置与下拉选择显示 (#3142) @ShadowLemoon
...
### 地图追踪
- 使用 YOLO 进行莉奈娅挖矿，优化配置与容��� (#3058) @zaodonganqi
...
- 修复纪行可选奖励导致流程卡住问题，并补充日志提�� (#2995) @ddaodan
...
- 文档链接��一增加 www 前缀 (#2996) @1004452714
```
...
## 其他
- **修复JSON��列化问题，本质原因是OCR模块指针指向的内存出现错误，感谢群友 焦糖玛奇朵
 提供环境协助定位！**。
...
- 给没有跟随主题的弹窗UI加上跟���主题的功能 (#2461) @ShadowLemoon
...
- 修复自定义的交互按键未���自动秘境生效的问题 (#2035) @Patrick-Ze
- 地图追踪中新增使用小道具的action
...
- 调度器添���地图追踪文件时自动聚焦搜索框 (#1916) @Arkadegong
...
- 修复地图追踪配置中��取材料名称的问题 (#1919) @kaedelcb
- 地图追踪版本要求修改为强制
- 优化地图追踪的距离过远判断
...
🌩️ [Github 下载太慢？ 点��访问更多更新渠道](https://bettergi.com/download.html)
...
![image](https://github.com/user-attachments/assets/47b54ac1-b524-4f62-a7a0-8e2b83221cf0)
    ...
    * 解决方案：关掉后重试一次，手动���行BetterGl.update.exe（此时打开软件应该已经是最新版了）。如果还是不行，可以直接重新下载安装包安装。
...
* 合成浓缩树脂：添加合成浓���后保留原粹树脂数量的功能 @kaedelcb
...
## 自动拾取
- **拾取列表已满时候，滑动滚轮（注意这个变更会会导致拾取变成从下往上拾取，对于尘歌壶收菜���一定影响）**
...
- 识别���择队伍后队伍切换成功的场景，减少因为切换时间过长而引发的队伍切换失败的问题 #1457
...
- 删除WGC的缓存模式选项，使用读写锁保证WGC的��序转换与返回。（也就是说本次更新后，WGC截图方式也能正常使用地图追踪等功能）
...
- 小改正则表达以支持JS��本统计 @MisakaAldrich #1274
- 优化一条路合成树脂时候容易失败的问题
...
* 修复路径追踪 `combat_script` 不设���角色名称仍会切人的问题 #1100
