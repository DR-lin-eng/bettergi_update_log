...
- 地图追踪识别过远或者失败时，则基于上个识别成功的点位进行��续 #2532
...
- 修复TemplateMatch��图特征加载两次的问题 (#2434) @JamisHoo
...
- 优化两处不致��的异常弹框展示 (#2464) @ShadowLemoon
...
- CountInventoryItem新��支持复数物品的计数，并在Dispatcher中向JavaScript暴露此种使用方式 (#2267) @FishmanTheMurloc
...
- 新增自动使用食物的Task���JS脚本调用 @FishmanTheMurloc
...
- 新增开发用的截取物品图标独立��务 (#1856) @FishmanTheMurloc
...
- 脚本仓库更新添加进度展示
- 优化传送点���击的逻辑，增加退出重登的延时 (#1773) @physligl
...
- 修复 JS 脚本内 log.debug 在 bettergi 覆盖��显示的日志级别不正确的问题 #1452
...
## 5.5 适配
- 使用新的游戏退出判断，影响自动伐木，���分JS脚本等 @physligl 
...
* win10不再展示毛玻璃主题切换按钮
* 完成秘境后释放所有按键 #1073
* 修复 `bettergi://start/` 不生效的问题（胡桃工具箱联动启动失效��题）
...
## 一条龙
* 把领取历练点任务改放在了领取奖励模块（在��换好感队之后） #955
* 修复枫丹台成台成女角色容易跑过头的问题
...
点击启动一条龙的是后，如果截图器未自动启动会直接启动，且会等待到开门后开始执行一条���。
...
* 进入对话中时会自动选择选项并退出。
* 修复路径追踪部分变量写死的问题
* 小于 1920x1080 的游��分辨率使用路径追踪会触发提示
...
* **队伍识别支持联机情况下的识别**
* 重新训练了头像分类器，使其支持角色头像处于半透明情况下的识别（攀爬、跳跃的时候右侧头像会变半透明，切人带数字的情况仍不��持）
* 修复后台自动剧情启用的情况下，自动领取探索奖励失效的问题
* 界面UI更新
...
```
BetterGI 距离第一个版本 0.10.0 的发布刚好已经过去了一年时间了，没想到能高强度开发一年，把 Github 的瓷砖都快刷全绿了。感谢各位开发者和群友们的帮���！
...
> **[minimap](https://github.com/tignioj/minimap) 是一个高精度原神自动采集工具，当然���的功能远不止这些，强烈推荐！！**
...
- 修复更新提示弹窗位置未适配高���屏DPI缩放 #446 @emako 
...
* 自动伐木默认不再开启OCR识别���材数量的功能（部分木材100%识别失败） #444
* 自动剧情添加了随机点击选项的选择
