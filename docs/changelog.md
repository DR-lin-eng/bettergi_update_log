...
```
本次更新完成原神 6.7 适配，新增自动首领讨伐、JSON ��斗策略与材料合成接口，并完善奖励识别、OCR 和遮罩体验。
...
### JS脚本
- 新增材料合成与背包物���计数接口，支持材料筛选、数量设置、产物识别及多物品计数 (#3231) @1004452714
...
- 扩展地图��辑器功能，支持直接运行和导出去除 BOM (#3090 #3092) @zaodonganqi
...
- 任务启动阶段可通过快捷键终止，强制关闭游戏��启动按钮可复位 (#3036) @zhousenyu
...
## 其他
- **Yap 的推理对���改为懒加载。index_2_word.json 修改成 Newtonsoft.Json 反序列化。修复 #2890 （也就是修复了这个报错：System.NotSupportedException: The type 'System.Int32' is not a supported dictionary key...）**
...
- 由于 TextBox 的 PlaceholderText 不展���，新增更多文字指引
...
- **地图特征使用二进制��件存储，减小文件体积，大幅提升特征数据加载速度** @shatyuka
...
- 自动剧情关闭弹出页前判断是否是大地图��弹出页面，以防误识别 #1466
