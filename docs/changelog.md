...
- 新增遮罩样式自定义并支持分辨率与 DPI 自适应，可分别调��日志和指标栏缩放 (#3184 #3352) @ddaodan @kaedelcb
...
```
本次更新完成原神 6.7 适配，新增自动首领讨伐、JSON 战斗策略与材料合成接口，���完善奖励识别、OCR 和遮罩体验。
...
- 跳过空月祝福使用硬���输入模拟，修复云原神不生效 (#3080) @KangKangTao
...
- 修复地图追踪触发自动剧情异��处理有时无法完成对话的问题 (#2684) @Anyrainel
...
- 适配 6.0 菈乌玛退队后队伍UI不会还原，但是跨地图传送后UI还�� 的场景
...
- 如果首个点是非TP点位，强制设置在这个点位附近优先做���部匹配，提升匹配速度
...
## 自动秘境
- 一条龙未选择要刷的秘境时，直接提示
- 修复4K分辨率下���境无法自动点击掉地脉异常的提示内容的问题 #460
