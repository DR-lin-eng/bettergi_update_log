...
- 新增自动剧情点击对话框前的延迟设置
- 减少配置组中两个任务中间的等待时�� @JamisHoo (#2590)
...
- 自动秘境浓缩树脂识别优化
- 自动音乐专辑任务异常详情日志打印
- 添加“打开所在目录”菜单项及其命令实现，���于打开脚本文件夹 (#2193) @bling-yshs
- 更新软件图标尺寸 @bling-yshs
...
- 修���并发调用小地图识别时识别错误的Bug，修改并增加暴露给JS的小地图识别函数 (#1911) @Limint
...
- 优化PromptDialog对话框布局以支持用户自由调整窗口大��� (#1953) @Datouyu666
...
## 底层改造
* OCR平台改造。移除 PaddleSharp ，使用 ONNX 直接进行��理，支持更多推理环境 #1459 @Takaranoao 
...
- **地图特征使用二进制文件存储，减小文件体积，大幅提升特征数据加载��度** @shatyuka
...
- 发送通知时，添��延迟让图片先发送，时间戳与事件之间添加换行 @MisakaAldrich
...
```
* 修复手动设置cd不生效和快速跳过战��时有时会导致刷屏的问题(#1410) @Takaranoao
...
* 修复 BitBlt 会��溃的问题，以及 BitBlt 性能的极致优化 @Takaranoao
* WGC帧率控制
...
```
> [!CAUTION]
> 由于本���更新修改了底层截图器与任务调度，这并不是一个稳定的版本，请谨慎选择更新！
...
* 感谢 @xyToki 提供的自���更新程序与服务，项目 [kachina-installer](https://github.com/YuehaiTeam/kachina-installer) ！
* 感谢 @Womsxd 提供的额外流量节点！
...
**在自动音游的基础上，新增自动专辑功能。在专辑界面下运行可以自动演奏完所有未领取完奖励的乐��。**
...
* 重构地图传送模块，解决部分场景下无法正常传送到目标的问题 @physligl 
    * 完善并修复传送点数据，补充了地区属性
* 完善自动战斗任务，添加了更���自定义参数，使自动锄地功能更加流畅 @mfkvfhpdx 
...
* 修复了角色在自动秘境结束战斗时，若朝向正西方��无法自动转向古树的问题。 #608
...
- 修复更新���示弹窗位置未适配高分屏DPI缩放 #446 @emako 
...
* 自动伐���默认不再开启OCR识别木材数量的功能（部分木材100%识别失败） #444
* 自动剧情添加了随机点击选项的选择
