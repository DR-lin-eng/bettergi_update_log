...
- **自动幽境危战适配5.8，并加入一条龙中（注意当前无需���开启钥匙后的界面上启动本功能！）**
...
- 调度器添加地图追踪文件时自动��焦搜索框 (#1916) @Arkadegong
...
- 内嵌 VC++ 运行库 14.44.35208.0，**尝试彻底解决“Unable to load DLL 'onnxruntime'or one of its dependencies: 动���链接库(DLL)初始化例程失败” 的问题**
...
![image](https://github.com/user-attachments/assets/47b54ac1-b524-4f62-a7a0-8e2b83221cf0)
    ...
    * 解决方案：关掉后重试一次，手动运行BetterGl.update.exe（此���打开软件应该已经是最新版了）。如果还是不行，可以直接重新下载安装包安装。
...
* 合成浓缩树脂：添加合成浓缩后保留原粹树脂数量��功能 @kaedelcb
...
- 识别选择队伍后队伍切换成��的场景，减少因为切换时间过长而引发的队伍切换失败的问题 #1457
...
* JS 脚本新增文件写入、输入文字的能力
* ���复 关闭截图器，切换分辨率，开启截图器后遮罩窗口位置不正常 的问题
...
- 删除WGC的缓存模式选项，使用读写锁保证WGC的有序��换与返回。（也���是说本次更新后，WGC截图方式也能正常使用地图追踪等功能）
...
* 残血、死亡时自动传送至七天神像
* 恢复界面上单独执行任务的右键菜单
* 添加路线重试机制、卡死脱���3次后放弃当前路径进入重试、卡死脱离添加超时时间
...
> [!IMPORTANT]
> **地图追踪���能**：自动传送到最近的锚点，然后根据小地图开始寻路。系统会根据任务文件的途径点和目标点进行导航。
...
1. **建议在游戏内配合快捷键使用**。
2. 录制时遇到主界面和���他界面的切换时，不要操作，需要等待程序识别这个切换过程
...
#### 如果在游戏大小为2K、4K且��图方式为`WindowsGraphicsCapture`的情况下，出现卡顿、鼠标移动缓慢等问题， 参考：https://github.com/babalae/better-genshin-impact/issues/286 关闭位图缓存。或者直接切换截图方式为`BitBlt`，Win11 `BitBlt`不能用？参考设置：https://github.com/babalae/better-genshin-impact/issues/92
