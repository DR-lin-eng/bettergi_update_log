...
- 新增可配置遮罩指标栏，可显示游戏帧率、处理耗时、跳过次数及硬件占���，并支持选择指标和拖拽调整布局 (#3121) @ddaodan
...
- 修改自动战斗ready指令的表现 
- 修复命令行启动时自动更新阻塞导��� StartGameTask 跳过等待主界面的问题 (#2902) @Shadow-Lemon
...
## 新功能
- **新增后台角色技能CD显��功能**，在遮罩窗口显示后台角色的技能冷却时间 (#2754) @zaodonganqi
...
- 修�� JS 脚本 multi-checkbox 配置项删除后仍能读取到旧值的问题 (#2725) @bling-yshs
...
- 优化地图追踪过程中各种打断的��理 (#2425) @JamisHoo
...
- 调整浓缩树脂识别范围，解决V4模型无法准���识别浓缩树脂个数的问题 #2185
...
- 切换队伍适配新版本最大队伍数由15增加到20，并新增最后���个非空队伍判断 @FishmanTheMurloc
...
- 删除 “允许在jsScript脚本中使用此地图追踪配置” 和 “允许在此调度器中（一般在JS脚本中）调用自动战斗任务时，采��此追踪配置里的战斗策略” 配置。默认启用
- 脚本仓库更新添加进度展示
...
## 地图追踪
* 通过点位配置，支持在未识别点位��况下，从大地图中心点来识别坐标。 #1526 @mfkvfhpdx
* 挖矿策略修改为只执行一个角色
...
## 5.5 适配
- 使用新的游戏退出判断，影响自���伐木，部分JS脚本等 @physligl 
...
* 优化一条龙领取奖励时，橙色选项的识别率
* 修改所有自动战斗的键鼠模拟方式，尝试���复自动战斗原地不动的问题
* 部分UI改造，部分文案修正
```
...
* 感谢 @xyToki 提供的自动更新程序与��务，项目 [kachina-installer](https://github.com/YuehaiTeam/kachina-installer) ！
* 感谢 @Womsxd 提供的额外流量节点！
