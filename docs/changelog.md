...
> 最后更新: 2026/9/26 20:45:43
> 数据来源: [GitHub Releases](https://github.com/babalae/better-genshin-impact/releases)

## 0.65 适配与优化

**版本:** `0.65.0`
**发布时间:** 2026/9/17 01:16:57
**GitHub页面:** [查看详情](https://github.com/babalae/better-genshin-impact/releases/tag/0.65.0)
...
- 优化遮罩窗口性能与状态切��：未启用指标时不再采样，日志框隐藏后仍可用快捷键恢复 (#3606 #3638) @guamian1337 @RRRR623
...
**新增自动演奏，支持多种曲谱格式、筛选播放与��动切换乐器 (#3419)**
...
- **重构传送任务的滑动���式与候选点位寻找** (#3258) @physligl
...
- 自动秘境和自动首领讨伐新增奖励名称��数量识别、跨轮汇总及 JS 返回结果支持 (#3208) @1004452714
...
- **自动剧情，通过Silero Vad识别对话结束，然后再选项��择 (#3130)** @ShadowLemoon
...
- 增加对话区判定次数 #2936 
- 优化TpTask惯���导航（误识别），处理初始未识别 (#2920) @physligl
- 切换角色问题测试用代码 
- 一条龙结束后关闭开书 #2686 
- 最后匹配自定义游戏进程名 #2909 
...
## 其他
- **Yap 的推理对象改为懒加���。index_2_word.json 修改成 Newtonsoft.Json 反序列化。修复 #2890 （也就是修复了这个报错：System.NotSupportedException: The type 'System.Int32' is not a supported dictionary key...）**
...
当前只���持提瓦特区域地表地图，不支持分层与独立地图。仅支持16:9的游戏分辨率
...
## 其他
- **同时启动原神的功能，添加使用CMD启动游戏的入口，如果你出现“检测到非法工具，请重��机器”的提示，可以尝试此功能**
...
- 优化���动战斗出战角色识别，修复部分场景下无法正常识别出战角色的问题
- 延长秘境载入判断时间 #2470
...
- 修复 AutoDomainParam 泛��定义 @breadgrocery
...
- 适配 6.0 菈乌玛退队后队伍UI不会还原，但是跨地��传送后UI还原 的场景
...
- JS脚本中 KeyPress ��法针对功能键也采取直接触发的方式 #2071
...
- 检测战斗结束增加旋转寻找敌���实验性功能 (#1960) @kaedelcb
...
* 修复其他树脂存在，但是原��树脂耗尽的情况下，会自动退出秘境的问题
...
* 修��地图追踪中的 up_down_grab_leaf 四叶印 action 不生效的问题
