...
需要在开启钥匙的界面上启动本功能（右下角��“开启挑战”按钮），主要是难三刷取圣遗物用
...
- 删除 “允许在jsScript脚本���使用此地图追踪配置” 和 “允许在此调度器中（一般在JS脚本中）调用自动战斗任务时，采用此追踪配置里的战斗策略” 配置。默认启用
- 脚本仓库更新添加进度展示
...
- 修复提���框内容过长时，对话框无法完整显示的问题 (#1513) @breadgrocery
- 优化一些报错和日志提示
- 支持5.6新角色的识别
...
- 新增关于页
- 迁移按键绑定页面到设置中
- 自动钓��恢复 16:9 的支持，**与一些钓鱼功能的优化** @FishmanTheMurloc 
- 一些UI界面上的变化
...
* **修复通知导致的截图失败报错提示**
* 使 JS脚�� 中的 keyDown(), keyUp(), keyPress() 方法 和 战斗策略脚本 中的 keydown(), keyup(), keypress() 方法 可以直接使用 虚拟键代码 来执行 鼠标操作 #1124 @2696791698
...
**添加了脚本仓��，在线版 [https://bgi.sh/](https://bgi.sh/) （感谢 @DR-lin-eng 友情提供的域名），BetterGI本地也内嵌了相同的网页。**
...
* 自动伐木默认不再��启OCR识别木材数量的功能（部分木材100%识别失败） #444
* 自动剧情添加了随机点击选项的选择
...
Paddle 文字识别暂时不能切换（测试切换后效果非常差）。图���推理相关的速度有大幅提升。
...
如果你平时使用BitBlt截图模式无法正常使用BetterGI，且自动活动音游功能也无法使用，可以尝��此方法： https://github.com/babalae/better-genshin-impact/issues/92
...
## 快捷键
- 快捷键新增键鼠监听模式，**支持任意键盘单键、鼠标侧键（XButton1、XButton2）**，不支持组合键，在更好的原神启动后���生效。#6  #45 #123 #228
...
- 修复自动战斗结束时，战斗任务被取消，部分行动���键按下后没有松开的问题
...
## 自动战斗与秘境
战斗
- **战斗脚本添加宏类方法，比如：click, keydown, keyup 等，具体使用方法见[高级语法](https://bgi.huiyadan.com/feats/domain.html#%E6%88%98%E6%96%97%E7%AD%96%E7%95%A5%E8%84%9A%E6%9C%AC-%E9%AB%98%E7%BA%A7%E8%AF%AD%E6%B3%95)**
