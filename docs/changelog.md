...
- 强制OCR使用CPU的功能默认关闭
- CUDA目录嗅探默认关闭&尝试修���在奇奇怪怪权限的环境下嗅探CUDA目录导致异常的问题 (#1994) @Takaranoao
- 推理引擎新增 OpenVINO 支持
...
- 重构添加地图追踪任务���面的搜索功能，同时添加深度搜索功能。修复了搜索功能形同虚设，只能搜索最表面父文件夹的问题。 (#1980) @Datouyu666
...
需要在开启钥匙的界面上启动本功��（右下角是“开启挑战”按钮），主要是难三刷取圣遗物用
...
点击启动一条龙的是后，如果截图���未自动启动会直接启动，且会等待到开门后开始执行一条龙。
...
## 自动剧情
* 自动交付物品支持多物品
* 添加更多NPC的��话选项排除（暂停选择） #350 @snouker
* 添加不自动选择选项的功能选项 #350 
