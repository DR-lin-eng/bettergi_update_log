...
<img width="650" height="914" alt="image" src="https://github.com/user-attachments/assets/ccf6e046-fa52-4c63-a834-6be31ef24a32" />
...
- 修复自定义的交互按键���对自动秘境生效的问题 (#2035) @Patrick-Ze
- 地图追踪中新增使用小道具的action
...
- 自动秘境换队从��门口切换修改为在进门后切换；原粹识别优化 (#1866) @kaedelcb
...
- 修改部���角色挖矿的方式 @RRRR623 @MisakaAldrich
...
## 一条龙
* 自动秘境：添加周日材料本三种奖励选择、优化进入秘��横幅点击 @kaedelcb
...
- 修复创建配置组添加js脚本后，首次编辑js脚本��定义配置时，组件无法正确绑定到setting.json中的默认值 (#1449) @breadgrocery
- 调用切换队伍方法后，清除队伍缓存
...
- **新增支持独立地图层岩巨渊、渊下宫，录制编辑器和追��地图查看同步支持**
...
* 添加使用Starward记录��戏时间的功能 @kndxhz #1164
...
* 联机情况下识���角色队伍时，修复错误识别房主的问题（会导致队伍识别出错） #777
...
* 残血、死亡时自动传送至七天神像
* 恢复界面上单独执行任务的右键菜单
* 添加路线重试机制、卡死脱离3次后放��当前路径进入重试、卡死脱离添加超时时间
...
* TP时，当前地图位置识别失败的��况下，会直接通过国家位置进行大地图定位
* TP失败时加入重试机制
...
* 修复伐木上限配置无法突破2000���问题（荒泷一斗被动可以突破这个值）。 #606
...
**感觉自动拾取有掉帧的朋友可以切换到GPU进行推理，然后自动拾取文字识���引擎选择 Yap，这样 CPU 占用会大幅降低。**
...
```
## 新功能
**新增了自动烹饪的功能，在实时任务中开启（不用的时候请关闭，比较消���性能）** #266
...
## 其他优化
* **修复非16:9屏幕在 0.29 版本新出现的问题：无法正常使用部分功能、遮罩识别框位���不正确**
...
> * 邀约分支修复了 **珐露珊、五郎、凝光、重云、鹿野院平藏** 分支不正确的问题，需要手动覆盖配置文件��效
