...
## 地图追踪 & 自动战斗
- **新的队伍编号识别方式，解决菈乌玛UI造成角色切换相关的问题，修复一键宏无法正常使用的��题**
- **6.0分层传送锚点、地下地图**
...
## 底层改造
* OCR平台改造。移除 PaddleSharp ，使用 ONNX 直接进���推理，支持更多推理环境 #1459 @Takaranoao 
...
- **地图特征使用二进制文件存储，减小文件体积，大幅提升特征数据加���速度** @shatyuka
...
- 发送通知时，���加延迟让图片先发送，时间戳与事件之间添加换行 @MisakaAldrich
...
```
* 修复手动设置cd不生效和快速跳过���斗时有时会导致刷屏的问题(#1410) @Takaranoao
...
* 修复 BitBlt ���崩溃的问题，以及 BitBlt 性能的极致优化 @Takaranoao
* WGC帧率控制
...
* 感谢 @xyToki 提供的���动更新程序与服务，项目 [kachina-installer](https://github.com/YuehaiTeam/kachina-installer) ！
* 感谢 @Womsxd 提供的额外流量节点！
...
**在自动音游的基础上，新增自动专辑功能。在专辑界面下运行可以自动演奏完所有未领取完奖励的��曲。**
...
Paddle 文字识别暂��不能切换（测试切换后效果非常差）。图像推理相关的速度有大幅提升。
