...
```
- **修复一键宏卡死问题** @cookie981
- 优化一条龙每周秘��配置与下拉选择显示 (#3142) @ShadowLemoon
...
- **修复JSON序列化问题，本质原因是OCR模块指针指向的内存出现错误，感谢群友 焦糖玛奇朵
 提供环境协助定位！**。
- **删除多余的启动游戏逻辑，解决启动一条龙��弹出多游戏启动失败的警告的问题** #2943 #2942 
- 检查游戏内小地图锁定配置 #1371 
- OCR修改默认强制推理配置 
...
- 修复角色识别失败��存图片路径不正确的问题 (#2354) @zaodonganqi
...
- 挪德卡莱中心点更新（修��不会自动选择挪德卡莱的问题）/联机时不执行自动派遣 (#2323) @kaedelcb
...
- 强制OCR使用CPU的功能默认关闭
- CUDA目录嗅探默认关闭&尝试修复在奇奇怪怪权限的环境下嗅探CUDA目录导致异常的���题 (#1994) @Takaranoao
- 推理引擎新增 OpenVINO 支持
...
- 重构添加地图追踪任务页面的搜索功能，同时添加深度搜索功能。修复了��索功能形同虚设，只能搜索最表面父文件夹的问题。 (#1980) @Datouyu666
...
## 通知
- 给telegramNotifier添加代理���能支持 (#1890) @xoipz
...
- 支持配���使用G键进行自动拾取 (#1502) @Patrick-Ze
...
🌩️ [Github 下载太慢？ 点我访��更多更新渠道](https://bettergi.com/download.html)
...
* JS 脚本新增文件写入、输入文字的能力
* 修复 关闭截图器，切换分辨率，开启截图器后遮罩���口位置不正常 的问题
...
- 删除WGC的缓存模��选项，使用读写锁保证WGC的有序转换与返回。（也就是说本次更新后，WGC截图方式也能正常使用地图追踪等功能）
...
* 修复脚本订阅和配置组运行的提示框在高DPI显示屏下跑��屏幕外的问题
...
```
**这次更新新增了新版本的安装程序与自动更新程序（自动更新程序下版本才会有��）。**
