...
- 修复角色索引识别偏差导致的���换问题 (#3035) @JerryChen97
...
- 原神 HDR 自动关闭功能
- 一条龙任务���增"关闭软件"完成动作 (#3115) @lingse4
...
## JS相关
- 支持调用 C#中含out/ref参数的方��� (#2884) @1004452714
...
## 地图追踪
- 优化换队拾取，仅在配置了万叶队伍时才执行换队��作 (#2748) @this-Fish
...
- ���进GetGridItemIconText获得全角数字字符时的处理 (#2581) @Patrick-Ze
...
- 调整浓缩树脂识别范围，解决V4模型无法准确��别浓缩树脂个数的问题 #2185
...
- 自动秘境换队从在门口切换修改���在进门后切换；原粹识别优化 (#1866) @kaedelcb
...
* win10不再展示毛玻璃主题切换按钮
* 完成秘境后释放所有按键 #1073
* 修复 `bettergi://start/` 不生效的问题（胡桃工具箱联动启���失效问题）
...
## 一条龙
* 把领取历练点任务改放在了领取奖励模���（在切换好感队之后） #955
* 修复枫丹台成台成女角色容易跑过头的问题
