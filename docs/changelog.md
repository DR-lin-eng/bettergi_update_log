...
```
原神 6.6 版本适��（新角色、空之神殿地图、传送点、七圣召唤），莉奈娅挖矿改用 YOLO 模型等
...
## 新功能
- **新增后台角色技能CD显示���能**，在遮罩窗口显示后台角色的技能冷却时间 (#2754) @zaodonganqi
...
- 新增自动剧情点击对话框前的延迟设置
- 减少配置组中两个任务中间的等��时间 @JamisHoo (#2590)
...
## 其他
- 优化自动剧情-关闭弹窗-小道具的检测���辑 (#2427) @1004452714
...
## 地图追踪 & 自动战斗
- **新的队伍编号识别方���，解决菈乌玛UI造成角色切换相关的问题，修复一键宏无法正常使用的问题**
- **6.0分层传送锚点、地下地图**
...
- 新增自动使用食��的Task供JS脚本调用 @FishmanTheMurloc
...
- 新增开发用的截取物品图��独立任务 (#1856) @FishmanTheMurloc
...
- 删除 “允许在jsScript脚本中使用此地图追踪配置” 和 “允许在此调度器中（一般在JS脚本中）调用自动战斗任务时，采用���追踪配置里的战斗策略” 配置。默认启用
- 脚本仓库更新添加进度展示
...
## 地图追踪
* 通过点位配置，支持在未识别点位情���下，从大地图中心点来识别坐标。 #1526 @mfkvfhpdx
* 挖矿策略修改为只执行一个角色
...
* win10不再展示毛玻璃主题切换按钮
* 完成秘境后释放所有按键 #1073
* 修复 `bettergi://start/` 不生效的问题（胡桃工具箱联动��动失效问题）
...
## 一条龙
* 把领取历练点任务改放在了领取奖励��块（在切换好感队之后） #955
* 修复枫丹台成台成女角色容易跑过头的问题
