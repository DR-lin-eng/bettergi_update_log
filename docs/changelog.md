...
- 新增遮罩样式自定义并支持分辨率与 DPI 自适应，可分别调整���志和指标栏缩放 (#3184 #3352) @ddaodan @kaedelcb
...
### 其他
- 完善首领讨伐的��条龙配置与战斗超时设置，并修复特殊地图体力识别、树脂弹窗和路线等待 (#3265 #3309 #3343 #3387) @1004452714 @Kirito520Asuna
...
- 新增 WinUI 风格导航选中��画并重写 Markdown 渲染组件 (#3369)
```
...
- 修复���动幽境危战切换队伍时可能误选首领的问题 (#3206) @bling-yshs
...
### 独立任务
- 地脉花新增"蒙德8-风车镇"路径，补充战斗中寻敌配置，修复战后切队与��束检测问题 (#3029 #3059 #3088) @ddaodan
...
- 地图遮罩实时任务内匹配操作异步化，不再阻塞自动��取等其他触发器
- 修复UI线程死锁的问题
...
## 其他
- **Yap 的推理对象���为懒加载。index_2_word.json 修改成 Newtonsoft.Json 反序列化。修复 #2890 （也就是修复了这个报错：System.NotSupportedException: The type 'System.Int32' is not a supported dictionary key...）**
...
- 使用 SetWinEventHook 优化日志遮罩针对原神窗口��跟随延迟 #2540
- 更多自动拾取黑名单
...
- 记录仓���页面位置和大小，提供默认值 (#2458) @zaodonganqi
...
## 其他
- 优化自动拾取的预处理环节，优化最终文字��现，使自动拾取OCR识别更加稳定
- **自动拾取排除聚所**
- 自动秘境浓缩树脂识别优化
- 自动音乐专辑任务异常详情日志打印
...
- 修复打开活动一览操作可能提前的问题
- 修复���境危战活动不在首位无法正常传送的问题 #2059 #2060
...
- 修复迷烟主成员非飞行状态下四叶印识别失���的Bug，模板匹配的置信度修改为数组 @Limint
...
* 将路径追踪相关的日志改为debug级�� (#1645) (#1684) @Patrick-Ze
...
* JS 脚本新增文件写入、输入文字的能力
* 修复 关闭截图器，切换分辨率，开启截图器后遮罩窗口位置���正常 的问题
...
* 修复地图追踪中的 up_down_grab_leaf 四叶印 action 不生效��问题
