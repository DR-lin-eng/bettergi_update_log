...
- 自动秘境浓缩树脂识别优化
- 自动音乐专辑任务异常详情日志打印
- 添加“打开所在目录”菜单项及其命令实��，用于打开脚本文件夹 (#2193) @bling-yshs
- 更新软件图标尺寸 @bling-yshs
...
- 优化PromptDialog对话框布局以支持用户自由调整窗��大小 (#1953) @Datouyu666
...
- 一条龙尘歌壶进入添加重���/UI微调/一条龙快捷键修复 #1750 #1752  @kaedelcb
...
- 打开地��时判断是否存在探索奖励，并自动领取（需要在设置中打开） @mfkvfhpdx 
...
- 修复提示框内容��长时，对话框无法完整显示的问题 (#1513) @breadgrocery
- 优化一些报错和日志提示
- 支持5.6新角色的识别
...
* 优化BitBlt截图逻辑，增加失败重试机���以提高截图成功率 (#1407) @Takaranoao
...
* 感谢 @xyToki 提供��自动更新程序与服务，项目 [kachina-installer](https://github.com/YuehaiTeam/kachina-installer) ！
* 感谢 @Womsxd 提供的额外流量节点！
...
**感��自动拾取有掉帧的朋友可以切换到GPU进行推理，然后自动拾取文字识别引擎选择 Yap，这样 CPU 占用会大幅降低。**
...
* 修复凯瑟琳每日自动领取委托/自动重新派遣关���功能无效的问题 https://github.com/babalae/better-genshin-impact/issues/417
...
```
## 新功能
**新��了自动烹饪的功能，在实时任务中开启（不用的时候请关闭，比较消耗性能）** #266
