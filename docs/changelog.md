...
- 适配 6.0 菈乌玛退队后队伍UI不会还原，但是跨地图传送后UI还原 的场���
...
- 添加火元素采集选项到条件定义 #2159
- 优化 地图追踪任务添���窗口 性能，修复了索引时文件树复选框不被记忆的BUG (#2157) @Datouyu666
...
- 降低飞行检测时间，自由落体后进入��行再下落攻击 (#1837) @MisakaAldrich
...
* 软件自带默认黑名单与用户配置黑名单拆分，不会再出现用户配��的黑名单被覆盖的情况了
* 配置黑白名单可实时生效
* 修复自动拾取展示文本可能缺失标点的问题
...
🌩️ [Github 下载太慢？ 点我访问更多��新渠道](https://bettergi.com/download.html)
```
...
* **WindowsGraphicsCapture 在 Windows 10 version 1903 以下���本中（不含）不再展示（因为系统不支持）** #394
