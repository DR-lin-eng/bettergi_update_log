...
```
本次更新完成原神 6.7 适配，新���自动首领讨伐、JSON 战斗策略与材料合成接口，并完善奖励识别、OCR 和遮罩体验。
...
### JS脚本
- ���增材料合成与背包物品计数接口，支持材料筛选、数量设置、产物识别及多物品计数 (#3231) @1004452714
...
- 新增基于怪物死亡经验判断��否执行战后拾取 (#3051) @kaedelcb
...
- **优化自动拾取对聚所的判断**
- **自动拾取添加模糊匹配的名单**
- 自动圣遗物转化功��可识别并标记未激活副词条 (#2258) @bhbghghbgb
...
- 修复自定义的交互按键未对自动秘境生效的��题 (#2035) @Patrick-Ze
- 地图追踪中新增使用小道具的action
...
- **自动幽境危战适配5.8，并加入一条龙中（注意当前���需在开启钥匙后的界面上启动本功能！）**
...
## 其他优化
- 优化抽屉关闭后重新开启的体验
- 一条���页面添加自动秘境树脂选择弹窗 #1767 @kaedelcb
...
## 调度器
* 调度器清空所有任务添加询问提示
* 新增shell类型的任务可供��加（**请不要执行看不懂的命令**）#1248 @Takaranoao
