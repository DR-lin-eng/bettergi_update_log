...
## 调度器与自动战斗
- 新增一条龙配置的重命名���删除 (#2658) @1004452714
...
- **文字菜单UI区域适配，修复自���领取尘歌壶在新版本报错的问题** (#2511) @kaedelcb
- 每日奖励领取适配
...
- 支持配置使用G键进���自动拾取 (#1502) @Patrick-Ze
...
* JS 脚本新增文件写入、输入文字的能力
* 修复 关闭截图器，切换分辨率，开启截图器后遮罩窗口位置不��常 的问题
...
* 修复地图追踪时其他界面关闭后没有延时，以及���地图界面不应该被关闭的问题
* 修复树脂不足提示检测 #1385
...
- 删除WGC的缓存模式选项，使用读写锁保证WGC的有序转换与返回。（也就是说本次更新后，WGC截图方式也能正常使用���图追踪等功能）
