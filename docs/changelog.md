...
- 适配月绽放角色（菈乌玛）入队后，角色无法正常切换的问题。（右侧队伍下方出现草露进度条，会让整个队伍UI上移，后续换队后UI依旧���持原样）
...
## 路径追踪
* 路径追踪 action为 fight 的��位强制执行成 path 类型
...
* 修复tp时候，屏蔽左上角区域位置不正确的问题（屏蔽了但没完全屏���）
* 新增在路径追踪期间使用小道具的功能
...
* 自动秘境功能存在角色死亡后，如果设置了目标秘境，则会自动传送回去��续自动秘境
...
## 自动战斗/秘境
* **大幅优化在自动战斗/秘境中队伍角色��识别成功率，在头像未处于半透明情况下识别率非常高（使用新训练的分类器替代原有的OCR方案）**
