...
- 彻底删除 实验功能 遮罩���原神子窗体方式启动 UseSubform
- 自动拾取排除：「月谕圣牌」收藏
...
- 新增自动剧情点击对话框前的延迟设置
- 减少配置组中两个任务���间的等待时间 @JamisHoo (#2590)
...
- 新增自��使用食物的Task供JS脚本调用 @FishmanTheMurloc
...
- 程序支持 attach 控制台输出
- 新增自动走向附近地脉花（宝箱图标）的Task，暂未��外 (#2073) @iris10086
...
- 新增开发用的截��物品图标独立任务 (#1856) @FishmanTheMurloc
...
## 地图追踪
* 通过点位配置，支持在未识���点位情况下，从大地图中心点来识别坐标。 #1526 @mfkvfhpdx
* 挖矿策略修改为只执行一个角色
...
* 重构地图传���模块，解决部分场景下无法正常传送到目标的问题 @physligl 
    * 完善并修复传送点数据，补充了地区属性
...
* **队伍识别支持联机情况下的识别**
* 重新训练了头像分类器，使其支持角色头像处于半透明情况下的识别（攀爬、跳跃的时候右侧头像会变半透明，切人��数字的情况仍不支持）
* 修复后台自动剧情启用的情况下，自动领取探索奖励失效的问题
* 界面UI更新
...
```
BetterGI 距离第一个版本 0.10.0 的发布刚好已经过去了一年时间了，没想到能高强度开发一年，把 Github 的瓷砖都快刷全绿了。感谢各位开���者和群友们的帮助！
