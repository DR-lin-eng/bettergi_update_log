...
```
本次更新完成原神 6.7 适配，新增自动首领讨伐、JSON 战���策略与材料合成接口，并完善奖励识别、OCR 和遮罩体验。
...
- 新增可配置遮罩指标栏，可显示游戏帧率、处理耗时、跳过次数及硬件占用，���支持选择指标和拖拽调整布局 (#3121) @ddaodan
...
- 新增自动剧情点击对话框前的延迟设置
- 减少配置组中两个任务中间的等待���间 @JamisHoo (#2590)
...
- 取消兑换码使用后不再弹出此剪切板���容的兑换码，新增兑换码剪切板监听的开关配置
...
- 调整官服退出重进参数，对高ping���家友好一点 (#1986) @Takaranoao
...
## 一条龙
* 自动秘境：添加周日材料本三种奖励选择、���化进入秘境横幅点击 @kaedelcb
...
> [!CAUTION]
> 注意在安装了 0.40.0 及更高版本的安装程序后，0.39.1 以���更低版本的安装程序（BetterGI_Setup_v*.exe）未来将会无法覆盖安装。如果要降低版本请使用便携版本（BetterGI_v*.7z）！
