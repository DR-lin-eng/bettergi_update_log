...
- **自动幽境危战适配5.8，并加入一条龙中（注意当前无��在开启钥匙后的界面上启动本功能！）**
...
- 内嵌 VC++ 运行库 14.44.35208.0，**尝试彻底解决“Unable to load DLL 'onnxruntime'or one of its dependencies: ��态链接库(DLL)初始化例程失败” 的问题**
...
![image](https://github.com/user-attachments/assets/47b54ac1-b524-4f62-a7a0-8e2b83221cf0)
    ...
    * 解决方案：关掉后重试一次，手动运行BetterGl.update.exe（��时打开软件应该已经是最新版了）。如果还是不行，可以直接重新下载安装包安装。
