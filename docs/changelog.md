...
### JS脚本
- 为 JS 脚本提供 HTML 遮罩功能，支持点��穿透模式切换 (#2975 #3100) @zaodonganqi @1004452714
...
- 优化二次拾取，万叶即使刚使用��技能也会强制拾取 (#2760) @this-Fish
...
- ���动拾取文本识别帧间连续性检查 (#2676) @zaodonganqi
...
- 修复TemplateMatch地图特征加载两次的问��� (#2434) @JamisHoo
...
- 提供自定义参数的 自动秘境 与 自动战斗 方�� (#2299) @1004452714
...
- 适配月绽放角色（菈乌玛）入队后，角色无法正常切换的问题。（右侧队伍下方出现草露进度条，会让整个��伍UI上移，后续换队后UI依旧保持原样）
...
* 软件自带默认黑���单与用户配置黑名单拆分，不会再出现用户配置的黑名单被覆盖的情况了
* 配置黑白名单可实时生效
* 修复自动拾取展示文本可能缺失标点的问题
...
![f4dc42c65d8902d2b1a01f08af05fa26](https://github.com/user-attachments/assets/0afdf0b9-e3b1-45ed-b0d3-3ab3f28b76e0)
    ...
    * 解决方案：请安装VC++运��库，官方下载地址： https://aka.ms/vs/17/release/vc_redist.x64.exe
...
- 修复创建配置组添加js脚本后，首次编辑js脚本自定义���置时，组件无法正确绑定到setting.json中的默认值 (#1449) @breadgrocery
- 调用切换队伍方法后，清除队伍缓存
...
- **新增支持独立地图层岩巨渊、渊下宫，录制编辑器和追踪地图���看同步支持**
...
- 新增关于页
- 迁移按键绑定页面到设置中
- 自动钓鱼恢复 16:9 的支持，**与一些钓鱼��能的优化** @FishmanTheMurloc 
- 一些UI界面上的变化
...
```
* **修复全自动钓鱼导致机器内存占满的问��** @FishmanTheMurloc 
...
```
**��次更新新增了新版本的安装程序与自动更新程序（自动更新程序下版本才会有用）。**
...
* 在未识别到配队选择页面时重试
* 新增命令行： `.\BetterGI.exe startOneDragon`，支持在软件启动后直接启动一条��� #998
...
* 自��秘境功能存在角色死亡后，如果设置了目标秘境，则会自动传送回去继续自动秘境
