...
```
本次适配原神7.0至冬内容，新增自动演奏和自动拾取白名单，并集中优化地图传送与稳定性。
...
- 获取剪贴板兑换码时忽略 URL，避免��链接内容误识别为兑换码 (#3470) @girl-dream
...
### 其他
- 完善首领讨伐的一条龙配置与战斗超时设置，并修复特殊地图体力识别、树脂弹窗和路线���待 (#3265 #3309 #3343 #3387) @1004452714 @Kirito520Asuna
...
- **修复JSON序列化问题，本质原因是OCR模块指针指向的内存出现错误，感谢群友 焦糖玛奇朵
 提供环境协助定位！**。
- **删除多余的启动游戏逻辑，解决启动一条龙时弹出多���戏启动失败的警告的问题** #2943 #2942 
- 检查游戏内小地图锁定配置 #1371 
- OCR修改默认强制推理配置 
...
- 地图追踪识别过远或者失败时，���基于上个识别成功的点位进行继续 #2532
...
- 优化自动战斗出战角色识别，修复部分场景下无法正���识别出战角色的问题
- 延长秘境载入判断时间 #2470
...
- ���过时的脚本仓库弹窗提醒，提供更新按钮并避免直接打开 (#2460) @ShadowLemoon
...
## 其他
- 本地仓库使用全新版本
- 优化渊下宫文字的识别，修复文本识���替换功能的问题 @JamisHoo
- 优化对进入主界面的判断
- 优化快速购买时候，对洞天的识别率
...
- 适��地图传送界面新出现“游逸旅闻：[地区名]”文字，对原有文字识别点击造成干扰的情况 @FishmanTheMurloc
...
## 一条龙
- 领取尘歌壶奖��优化，新增其他进入尘歌壶的方式 (#1797) @iris10086
...
- 地图追踪接��修改成多实例，JS调用的实例和地图追踪调用的实例不同
...
![image](https://github.com/user-attachments/assets/47b54ac1-b524-4f62-a7a0-8e2b83221cf0)
    ...
    * 解决方案：关掉后重试一次，手动运行BetterGl.update.exe（此时打开软件应该已经是最新版了）。如果还是不行，可以��接重新下载安装包安装。
...
* 使用TorchSharp重写自动钓鱼��抛竿算法（未启用） @FishmanTheMurloc
...
- 修复自动秘境超时的时候重试
- 遮罩窗口UI优化支���缩放，日志窗口添加阴影 (#1477、#1478)  @shatyuka
...
- ���改正则表达以支持JS脚本统计 @MisakaAldrich #1274
- 优化一条路合成树脂时候容易失败的问题
