...
- 修复��图追踪触发自动剧情异常处理有时无法完成对话的问题 (#2684) @Anyrainel
...
## 自动秘境/幽境/战斗
- 战斗策略中，上一个命令和当前命令不是同一��角色时，直接切换角色 #2411
...
- 修复��取尘歌壶奖励相关问题 (#2437) @breadgrocery
...
- 删除这两个选项：是否就近七天神像恢复血量、���送到七天神像之后是否需要移动后回血
...
## 底层改造
* OCR平台改造。移除 PaddleSharp ，使用 ONNX 直接进行推理，支持更多推���环境 #1459 @Takaranoao 
...
* 优化一条龙领取奖励时，橙色选项的识别率
* 修改所有自动战斗的键鼠模拟方式，尝试修复���动战斗原地不动的问题
* 部分UI改造，部分文案修正
```
...
* 添加 `log_output` 的 action 用��在指定点输出日志
