...
- 适�� 6.0 菈乌玛退队后队伍UI不会还原，但是跨地图传送后UI还原 的场景
...
- 支持配置使用G键进行自动���取 (#1502) @Patrick-Ze
...
* 自动战斗后、传送前都会释���所有按键，尝试解决TP神像后持续往前走的问题
* 修复火元素采集不生效的问题
* 优化自动传送的稳定性 @physligl
...
## 路径追踪
* 路径追��� action为 fight 的点位强制执行成 path 类型
...
## 独立任务
* 重构底层调度逻辑，使其方法能够对外调用
* **<kbd>F11</kbd> 关闭截图器的时候也��同时关闭所有独立任务** #388  #660
...
**自动调查离传送点较近的狗粮。请使用琳妮特前台，双风共鸣。并保证所���传送点都已经激活！**
...
#### 如果在游戏大小为2K、4K且截图���式为`WindowsGraphicsCapture`的情况下，出现卡顿、鼠标移动缓慢等问题， 参考：https://github.com/babalae/better-genshin-impact/issues/286 关闭位图缓存。或者直接切换截图方式为`BitBlt`，Win11 `BitBlt`不能用？参考设置：https://github.com/babalae/better-genshin-impact/issues/92
