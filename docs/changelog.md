...
- 回滚“文本识别模���匹配”相关改动，解决自动拾取卡的问题 (#2799) 
...
## 其他
- **修复JSON序���化问题，本质原因是OCR模块指针指向的内存出现错误，感谢群友 焦糖玛奇朵
 提供环境协助定位！**。
...
- 修复七圣召唤月之五��涅芙会导致手牌不可调和的死循环 (#2848) @Guest-Liang
...
- 修复 OCR RunMulti 中 Mat 所有权语义不明确导���的潜在 Dispose 问题 (#2730) @Ayu0K
...
## 其他
- 添加动态修改遮罩日志与状态位置的功能
- **修复当前出战队伍是第五位时，无法正常下拉切换���伍的问题** #2633
...
- 添加火元素采集选项到条件定义 #2159
- ���化 地图追踪任务添加窗口 性能，修复了索引时文件树复选框不被记忆的BUG (#2157) @Datouyu666
...
- 修复自定义的交互按键未对��动秘境生效的问题 (#2035) @Patrick-Ze
- 地图追踪中新增使用小道具的action
...
- **自动幽境危战适配5.8，并加入一条���中（注意当前无需在开启钥匙后的界面上启动本功能！）**
...
- 调度器添加��图追踪文件时自动聚焦搜索框 (#1916) @Arkadegong
...
## 一条龙
* 自动秘境：添加周日材��本三种奖励选择、优化进入秘境横幅点击 @kaedelcb
...
## 自动拾取
- **拾取列表已满���候，滑动滚轮（注意这个变更会会导致拾取变成从下往上拾取，对于尘歌壶收菜有一定影响）**
...
- 打开地图时判断是否存在探索奖励，并自动领��（需要在设置中打开） @mfkvfhpdx 
...
* 修复 BitBlt 会崩溃的问题，以及 BitBlt 性能的极致��化 @Takaranoao
* WGC帧率控制
...
* 修复路径追踪 `combat_script` 不设置���色名称仍会切人的问题 #1100
