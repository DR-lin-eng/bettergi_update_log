...
- 检测��斗结束增加旋转寻找敌人实验性功能 (#1960) @kaedelcb
...
- 降低飞行检测时间，自由落体后进入飞行���下落攻击 (#1837) @MisakaAldrich
...
* 软件自带默认黑名单与用户配置黑名单拆分，不会再出现用户配置的���名单被覆盖的情况了
* 配置黑白名单可实时生效
* 修复自动拾取展示文本可能缺失标点的问题
...
### 路径追踪
* 修复到目标点时不转动视角的问题
* 在识别到卡��时，新增更好的避障逻辑 & 优化寻路逻辑 #711 @jbcaaa 
...
**自动调查离传送点较近的狗粮。请使用琳妮特前台，双���共鸣。并保证所有传送点都已经激活！**
...
* **WindowsGraphicsCapture 在 Windows 10 version 1903 以下版本中（��含）不再展示（因为系统不支持）** #394
...
**识别思��来自群友 `@益蜘蛛` ，实现方法参考了另一位群友@DR-lin-eng编写的自动音游程序： https://github.com/DR-lin-eng/genshin-auto-music**
