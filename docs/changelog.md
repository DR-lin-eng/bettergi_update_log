...
```
原神 6.6 版本适配（新角色、空之神殿地图、传送点、七圣��唤），莉奈娅挖矿改用 YOLO 模型等
...
- 修复纪行可选奖励��致流程卡住问题，并补充日志提醒 (#2995) @ddaodan
...
- **修复最小化时候截图失败导致截图器没法正常截图，导致主界面卡��的问题** #2939  #2851 
...
- 修复��用ESM时，仅使用同步代码报错 (#2592) @breadgrocery
...
- 取消兑换码使用后不再弹出此剪切板内容的兑换码，新增兑换码剪切板���听的开关配置
...
- 修复配置组名称前半段有���名不显示的问题 (#1858) @kaedelcb
...
![f4dc42c65d8902d2b1a01f08af05fa26](https://github.com/user-attachments/assets/0afdf0b9-e3b1-45ed-b0d3-3ab3f28b76e0)
    ...
    * 解决方案：请安装VC++��行库，官方下载地址： https://aka.ms/vs/17/release/vc_redist.x64.exe
...
- 修复创建配置组添加js脚本后，首次编辑js脚本自定���配置时，组件无法正确绑定到setting.json中的默认值 (#1449) @breadgrocery
- 调用切换队伍方法后，清除队伍缓存
...
- **新增支持独立地图层岩巨渊、渊下宫，录制编辑器和追踪地���查看同步支持**
...
- 新增关于页
- 迁移按键绑定页面到设置中
- 自动钓鱼恢复 16:9 的支持，**与一些钓��功能的优化** @FishmanTheMurloc 
- 一些UI界面上的变化
