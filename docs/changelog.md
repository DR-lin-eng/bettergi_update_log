...
### 自动战斗
- 优化战斗结束检测，支持派蒙辅助识别和更���活的检查时机配置 (#3378) @Bedrockx
- 修复游泳检测提前释放截图对象导致的异常
...
- 优化全局异常处理和启动失败���底，修复非 UI 线程弹窗造成的内存泄漏 (#3454 #3461) @guamian1337
...
**新增[桌面分身](https://www.bettergi.com/feats/command/session.html)功能，你可以在桌面分身中���动 BetterGI 与游戏，可以做到游戏不占用前台鼠标！注意该功能设置上存在难度，请仔细阅读相关文档。**
...
- 优化幽境危战状态流转，兼容未解锁 N6 的账号并��确处理活动结束返回主界面 (#3330 #3331) @ShadowLemoon
...
- 尝试修复��送webhook请求时，screenshot字段值为空的问题 #2506
```
...
- 使用 SetWinEventHook 优���日志遮罩针对原神窗口的跟随延迟 #2540
- 更多自动拾取黑名单
...
- 重构邮件通知器以支持 465 端口和 587端�� (#2327) @xdd-bubble-tea
- 支持自定义脚本仓库地址 @ddaodan
...
- 秘境走到最后领取奖励超时将会自动重��秘境 #1083 #1202 #1270 
...
* 自动战斗后、传送前都会释放所有按��，尝试解决TP神像后持续往前走的问题
* 修复火元素采集不生效的问题
* 优化自动传送的稳定性 @physligl
