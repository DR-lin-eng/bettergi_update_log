...
- 取消兑换码使用后不���弹出此剪切板内容的兑换码，新增兑换码剪切板监听的开关配置
...
* win10不再展示毛玻璃主题切换按钮
* 完成秘境后释放所有按键 #1073
* 修复 `bettergi://start/` 不生效的问题（胡桃工具箱联动启动失���问题）
...
## 一条龙
* 把领取历练点任务改放在了领取奖励模块（���切换好感队之后） #955
* 修复枫丹台成台成女角色容易跑过头的问题
...
- 修复更新提示弹窗位置未适配��分屏DPI缩放 #446 @emako 
...
* 自动伐木默认不再开启OCR识��木材数量的功能（部分木材100%识别失败） #444
* 自动剧情添加了随机点击选项的选择
...
Paddle 文字识别暂时不能切换（测试切换后���果非常差）。图像推理相关的速度有大幅提升。
