...
## 自动拾取
- **拾取列表已满时候，滑动滚轮（注意这个变更会会导致拾���变成从下往上拾取，对于尘歌壶收菜有一定影响）**
...
* 修复地图追踪时其他界面关闭后没有延时，以及大地图���面不应该被关闭的问题
* 修复树脂不足提示检测 #1385
...
- 删除WGC的缓存模式选项，使用读写锁保证WGC的有序转换与返回。（也就是说本次更新后，WGC截图方式也能正常使用地图追���等功能）
