...
- 适配月绽放��色（菈乌玛）入队后，角色无法正常切换的问题。（右侧队伍下方出现草露进度条，会让整个队伍UI上移，后续换队后UI依旧保持原样）
...
- JS脚本、地图追踪页面markdown文件��载修改为WPF原生的展示方式 (#2017) @Datouyu666
...
- 自动秘境换��从在门口切换修改为在进门后切换；原粹识别优化 (#1866) @kaedelcb
...
- 修���部分角色挖矿的方式 @RRRR623 @MisakaAldrich
...
![f4dc42c65d8902d2b1a01f08af05fa26](https://github.com/user-attachments/assets/0afdf0b9-e3b1-45ed-b0d3-3ab3f28b76e0)
    ...
    * 解决方案：请���装VC++运行库，官方下载地址： https://aka.ms/vs/17/release/vc_redist.x64.exe
...
* 修复更新窗口色差以及白边的问题
* 修复暂停后恢复异常bug，调度���设置增加周期配置，执行调度器任务增加继续执行功能 @mfkvfhpdx
...
- 修复创建配置组添加js脚本后，首次编辑js��本自定义配置时，组件无法正确绑定到setting.json中的默认值 (#1449) @breadgrocery
- 调用切换队伍方法后，清除队伍缓存
...
- **新增支持独立地图层岩巨渊、渊下宫，录制编辑器��追踪地图查看同步支持**
...
## 其他优化
* **修复非16:9屏幕在 0.29 版本新出现的问题：无法正常使用部分功能、遮��识别框位置不正确**
