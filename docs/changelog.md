...
- 自动地脉花新增挪德卡莱 3「月矩力试验设计局」路径 (#2991) @ddaodan
- 优化新版自动秘境奖励通知时机
...
- 修改自动战斗ready指令的表现 
- ��复命令行启动时自动更新阻塞导致 StartGameTask 跳过等待主界面的问题 (#2902) @Shadow-Lemon
...
- 调整浓缩树脂识别范围，解决V4模型无法准确识别浓���树脂个数的问题 #2185
...
- 自动秘境浓缩树脂识别优化
- 自动音乐专辑任务异常详情日志打印
- 添��“打开所在目录”菜单项及其命令实现，用于打开脚本文件夹 (#2193) @bling-yshs
- 更新软件图标尺寸 @bling-yshs
...
- 由于 TextBox 的 PlaceholderText 不展示，新��更多文字指引
...
## 其他优化
- 优化抽屉关闭后重新开启的体验
- 一条龙页面添��自动秘境树脂选择弹窗 #1767 @kaedelcb
...
## 底层改造
* OCR平台��造。移除 PaddleSharp ，使用 ONNX 直接进行推理，支持更多推理环境 #1459 @Takaranoao 
...
- JS添��实时任务,不会清理之前的任务 @Takaranoao #1320 
...
## 调度器
* 调度器清空所有任务添加询问提示
* 新增shell类型的任务可供添加（**���不要执行看不懂的命令**）#1248 @Takaranoao
...
* 修改了��径追踪中战斗结束的判定点 @Scarlet1ssimo
...
* 重构地图传送模块，解决部��场景下无法正常传送到目标的问题 @physligl 
    * 完善并修复传送点数据，补充了地区属性
