...
- 程序支持 attach 控制台输出
- 新增自动走向附近地脉花（宝箱图��）的Task，暂未对外 (#2073) @iris10086
...
- ���增开发用的截取物品图标独立任务 (#1856) @FishmanTheMurloc
...
![f4dc42c65d8902d2b1a01f08af05fa26](https://github.com/user-attachments/assets/0afdf0b9-e3b1-45ed-b0d3-3ab3f28b76e0)
    ...
    * 解决方案：请安装VC++运行���，官方下载地址： https://aka.ms/vs/17/release/vc_redist.x64.exe
...
* 修复更新窗口色差以及白边的问题
* ���复暂停后恢复异常bug，调度器设置增加周期配置，执行调度器任务增加继续执行功能 @mfkvfhpdx
...
- 新增关于页
- 迁移按键绑定页面到设置中
- ��动钓鱼恢复 16:9 的支持，**与一些钓鱼功能的优化** @FishmanTheMurloc 
- 一些UI界面上的变化
...
**自动晶蝶** 是内置的第一个脚本，调用了内部的大地���识别并传送的功能，然后执行键鼠脚本。
...
为了实现 #95 做了底层重构，主要是为了在原来的基础上更好的支持高于1080P的分辨率，���如2k、4k。提高其处理性能。
...
为了解��自动拾取的高占用，新增了一个OCR识别引擎，识别速度稍慢但是占有低，并作为默认选项。解决此类卡顿问题：#383
...
## 快捷键
- 快��键新增键鼠监听模式，**支持任意键盘单键、鼠标侧键（XButton1、XButton2）**，不支持组合键，在更好的原神启动后才生效。#6  #45 #123 #228
...
- **新增了自动提交物��的功能**。由于没有找到多个物品的测试场景，所以现在只支持一个物品。
...
## 快速传送
- 更加精准的快速传送，**在存在多个传送点的情况下会选择第一���传送点**，当然消耗的性能也增加了
- 新增按住快捷触发快速传送的方式 #127
- 新增了快速传送的开关快捷键 #229 
