...
```
> [!CAUTION]
> 0.63 版本识图素材���层重构，可能存在较多未知 BUG ，当前版本并不稳定，请谨慎选择更新！
...
### 独立任务
- 地脉花新���"蒙德8-风车镇"路径，补充战斗中寻敌配置，修复战后切队与结束检测问题 (#3029 #3059 #3088) @ddaodan
...
- 挪德卡莱中心点更新（修复不会自动���择挪德卡莱的问题）/联机时不执行自动派遣 (#2323) @kaedelcb
...
- 强制OCR使用CPU的功能默认关闭
- CUDA目���嗅探默认关闭&尝试修复在奇奇怪怪权限的环境下嗅探CUDA目录导致异常的问题 (#1994) @Takaranoao
- 推理引擎新增 OpenVINO 支持
...
需要在开启���匙的界面上启动本功能（右下角是“开启挑战”按钮），主要是难三刷取圣遗物用
...
- 修复自动秘境选择火本（无妄引咎密宫）后，无��正确进入秘境的问题 #1345
...
* 修复地图追踪中的 up_down_grab_leaf 四叶印 action 不生效的��题
