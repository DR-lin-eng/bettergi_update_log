...
* 挖矿策略修改为只执行一个角色
* 大剑挖矿策��修改，以及移除卡齐娜 @MisakaAldrich
...
* 修复更新窗口色差以及白边的问题
* 修复暂停后恢复异常bug，调度器设置增加周期配置，执行调度器任务增加继续执行功�� @mfkvfhpdx
...
- 如果首个点是非TP点位，��制设置在这个点位附近优先做局部匹配，提升匹配速度
...
```
**这次更新新增了新版本的安装程序与自动更新���序（自动更新程序下版本才会有用）。**
...
* 修复tp时候，屏蔽左上角区域位置不正确的问题（屏蔽了但��完全屏蔽）
* 新增在路径追踪期间使用小道具的功能
...
* 自动秘境功能存在角色死亡后，如果设置了目标���境，则会自动传送回去继续自动秘境
...
只支持最大的地表地图，不支持地底识别，也不支持渊下宫、层岩巨渊地底地图。由于自带了原神地图的全特征点文件，软件体积暴��。
...
由于键鼠脚本分享时，不同电脑存��很多影响变量，**测试发现存在大量无法正确重放的情况**。
...
**如果期望全连建议��择胡桃主题“疏影引蝶映梅红”，背景效果调整为1**
...
## 自动战斗/秘境
* **大幅优化在��动战斗/秘境中队伍角色的识别成功率，在头像未处于半透明情况下识别率非常高（使用新训练的分类器替代原有的OCR方案）**
...
* 0.24 版本 WindowsGraphicsCapture 截图方式的优化作为可配置项，如果有掉帧情况，可以尝���在“启动”页关闭此优化功能
* 重构了部分脏代码 @emako 
...
- **新增了���动提交物品的功能**。由于没有找到多个物品的测试场景，所以现在只支持一个物品。
