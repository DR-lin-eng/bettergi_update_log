...
* 修复路径追踪 `combat_script` 不设置角色名称仍���切人的问题 #1100
...
- 自动秘境指定轮数完成后，退出副本
- 所有独立任务都新增不可���入的互斥锁，同一时间只能存在一个独立任务正在运行 #304
- 自动伐木内存回收优化 #311
...
- **在键鼠监听模式下，长���快捷键再也不会停不下来了，龙王转圈也不会出现卡顿**
