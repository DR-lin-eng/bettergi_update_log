...
当前只支持提瓦特区域地表地图，不支持分层��独立地图。仅支持16:9的游戏分辨率
...
- 检测战斗结束增加旋转寻找���人实验性功能 (#1960) @kaedelcb
...
* 修复其他树脂存在，但是��粹树脂耗尽的情况下，会自动退出秘境的问题
...
```
**这次更新新增了新版本的安装程序与自动更新程序（自动��新程序下版本才会有用）。**
...
## 路径追踪
* 路径追踪 action为 fight 的点位���制执行成 path 类型
...
* 自动秘境功能存在角色死亡后，如果设置了目标秘境，则会��动传送回去继续自动秘境
