...
- **优化自动拾取对聚所的判断**
- **自动拾取添加模糊匹配的名单**
- 自动圣遗���转化功能可识别并标记未激活副词条 (#2258) @bhbghghbgb
...
- 内嵌 VC++ 运行库 14.44.35208.0，**尝试彻���解决“Unable to load DLL 'onnxruntime'or one of its dependencies: 动态链接库(DLL)初始化例程失败” 的问题**
...
- 打开地图��判断是否存在探索奖励，并自动领取（需要在设置中打开） @mfkvfhpdx 
...
- 修复提示框内容过��时，对话框无法完整显示的问题 (#1513) @breadgrocery
- 优化一些报错和日志提示
- 支持5.6新角色的识别
...
* 优化BitBlt截图逻辑，增加失败重试机制���提高截图成功率 (#1407) @Takaranoao
...
由于键鼠脚本分享时，不同电脑存在很多影响变量，**测试发现存��大量无法正确重放的情况**。
