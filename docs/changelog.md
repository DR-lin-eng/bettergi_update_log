...
- 自动地脉花���立战斗配置与OCR遮罩 (#2829) @ddaodan
...
- 修复 OCR RunMulti 中 Mat 所有权语义不明确导致的潜在 Dispose ��题 (#2730) @Ayu0K
...
- 配置弹窗UI优化，更��截取物品图标导航地址 (#2499) @zaodonganqi
...
- 适配更多 6.0 队伍相关改��� @kaedelcb @FishmanTheMurloc
...
* 支持在命令行启动一条龙参数时选择配置，配置找不到就用界面指定��� #1586 @zzc-tongji
...
## 自动拾取
- **拾取列表已满时候，滑动滚轮（注意这个变更会会导致拾取变成从下往上拾取，对于尘歌壶收��有一定影响）**
...
- 识��选择队伍后队伍切换成功的场景，减少因为切换时间过长而引发的队伍切换失败的问题 #1457
...
- 优化一些报错和日志提示
- 支持5.6新角色的识别
- 修复一��UI上数据绑定的问题 @FishmanTheMurloc
...
- 删除WGC的缓存模式选项，��用读写锁保证WGC的有序转换与返回。（也就是说本次更新后，WGC截图方式也能正常使用地图追踪等功能）
...
- 小���正则表达以支持JS脚本统计 @MisakaAldrich #1274
- 优化一条路合成树脂时候容易失败的问题
