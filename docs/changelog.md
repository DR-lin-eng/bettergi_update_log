...
- 调度器添加地图追踪文件时自动聚��搜索框 (#1916) @Arkadegong
...
## 自动拾取
- **拾取列表已满时候，滑动滚轮（注意��个变更会会导致拾取变成从下往上拾取，对于尘歌壶收菜有一定影响）**
...
- 增加对WaypointForTrack的DisablePickupAround属性���持，可以关闭挖矿动作的周围拾取 @Takaranoao #1322
...
* 自动战斗后、传送前都会��放所有按键，尝试解决TP神像后持续往前走的问题
* 修复火元素采集不生效的问题
* 优化自动传送的稳定性 @physligl
...
* 修复路径追踪 `combat_script` 不设置角色名称仍会切人的问�� #1100
...
- 键鼠脚���的录制与回放从相对时间修改为绝对时间，大幅提高精度，减少卡顿，**但是不兼容 0.33 版本前的键鼠脚本** @lwh9346 
...
**感觉自动拾取有掉帧的朋友可以切换到GPU进行推理，然后自动拾���文字识别引擎选择 Yap，这样 CPU 占用会大幅降低。**
