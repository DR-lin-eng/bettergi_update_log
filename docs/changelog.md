...
- 修复自定义的交互按键未对自动��境生效的问题 (#2035) @Patrick-Ze
- 地图追踪中新增使用小道具的action
...
- 自动秘境换队从在门口切��修改为在进门后切换；原粹识别优化 (#1866) @kaedelcb
...
- 如果首个点是非TP点位，强制设置在这个点位附近优先做局部匹配，��升匹配速度
...
- 一些UI界面上的变化
- wpfui 更新至 4.0 (表层没啥变化，��是ui框架底层变动)
...
* 修复地图追踪���的 up_down_grab_leaf 四叶印 action 不生效的问题
...
由 @HZYgrandma 贡献，由于使用的是和自动晶蝶一样的自动传送+键鼠录制的方式，长距离可能无法精准到达点位。且由于键鼠录制不够稳定，出现无法正确拾取狗粮的情况属于正常情况。后续可能���通过地图追踪功能重写。
...
Paddle ��字识别暂时不能切换（测试切换后效果非常差）。图像推理相关的速度有大幅提升。
