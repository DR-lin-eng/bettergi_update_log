...
* 在未识别到配队选择页面时重试
* 新增命令行： `.\BetterGI.exe startOneDragon`，��持在软件启动后直接启动一条龙 #998
...
## 其他优化
* Yap 自动拾取模型更新
* 修复原神启动时，`config.ini` 不生效的问题（用了官B切换���具，但使用BetterGI启动时永远是官服）
