...
- 新增可配置遮罩指标栏，可显示游戏帧率、处理耗时、���过次数及硬件占用，并支持选择指标和拖拽调整布局 (#3121) @ddaodan
...
- 优化二次拾取，万叶即使刚使用过技能��会强制拾取 (#2760) @this-Fish
...
- 由于 TextBox 的 PlaceholderText 不���示，新增更多文字指引
...
- 删除这两个选项：是否就近七天神像恢复血���、传送到七天神像之后是否需要移动后回血
...
- ���化PromptDialog对话框布局以支持用户自由调整窗口大小 (#1953) @Datouyu666
...
## 0.46 版本常见问题FAQ
* 0.45 更新到 0.46 时报错存在��字：**Error: NO_HASH_ALGO_ERR: No hash algorithm specified**。
