...
当前只支持提瓦特区域地表地图，不支持分层与独���地图。仅支持16:9的游戏分辨率
...
- 修复打开活动一览操作可能提前的问题
- 修复幽境危战活动不在首位无法正常传��的问题 #2059 #2060
...
- 修复迷烟主成员非飞行状态下四叶印识别失败的Bug，模板匹配的置信度修改为��组 @Limint
...
## 自动秘境
- 一条龙未选择要刷的秘境时，直接提示
- 修复4K分辨率下秘境无法自动点击掉地脉异常的提示内容的问��� #460
...
* **修复通知导致的截图失败报错提示**
* 使 JS脚本 中的 keyDown(), keyUp(), keyPress() 方法 和 战斗策略脚本 中的 keydown(), keyup(), keypress() 方法 可以直���使用 虚拟键代码 来执行 鼠标操作 #1124 @2696791698
...
**自动调查离传送点较近的狗粮。请使用琳妮特前台，双风共鸣。并保证所有传送��都已经激活！**
