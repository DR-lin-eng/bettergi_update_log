...
- 获取剪贴板兑换码时忽略 URL，避免把��接内容误识别为兑换码 (#3470) @girl-dream
...
- 自动幽境危战，增强状��机节点注册与重试兜底 (#3159) @ShadowLemoon
...
## 其他
- **修复JSON序列化问题，���质原因是OCR模块指针指向的内存出现错误，感谢群友 焦糖玛奇朵
 提供环境协助定位！**。
...
- 修复七圣召唤月之五伊涅芙会导��手牌不可调和的死循环 (#2848) @Guest-Liang
...
## 地图遮罩
- **优化地图���罩UI，添加快捷键** (#2775) @zaodonganqi
- 地图遮罩开关实时生效，关闭时立即隐藏
...
## 其他
- **同时启动原神的功能，添加使用CMD启动游戏的入口，如果你出现“检��到非法工具，请重启机器”的提示，可以尝试此功能**
...
- ���复 AutoDomainParam 泛型定义 @breadgrocery
...
- 提高领取纪行奖励的鲁棒���（实际是V5 OCR识别的问题） #2019 @JamisHoo
...
- 强制OCR使用CPU的功能默认关闭
- CUDA目录嗅探默认关闭&尝试修复在奇��怪怪权限的环境下嗅探CUDA目录导致异常的问题 (#1994) @Takaranoao
- 推理引擎新增 OpenVINO 支持
...
- 重构添加地图追踪任务页面的��索功能，同时添加深度搜索功能。修复了搜索功能形同虚设，只能搜索最表面父文件夹的问题。 (#1980) @Datouyu666
...
- 删除 “允许在jsScript��本中使用此地图追踪配置” 和 “允许在此调度器中（一般在JS脚本中）调用自动战斗任务时，采用此追踪配置里的战斗策略” 配置。默认启用
- 脚本仓库更新添加进度展示
