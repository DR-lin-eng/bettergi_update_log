...
## 自动拾取
* 优化文字识别功能，使用新的文字区域检测方式减少CPU消耗（**不会出现 100% CPU��情况了**）
...
* 支持在命令行启动一条龙��数时选择配置，配置找不到就用界面指定的 #1586 @zzc-tongji
...
- 为凌晨4点领取月卡的逻辑添���了更多触发点 @mfkvfhpdx
...
- 发送通知时，添加延迟让图片先发送，时间戳与事件之间添加��行 @MisakaAldrich
...
* 修复路径追踪 `combat_script` 不设置角色名称���会切人的问题 #1100
...
* 优化路径追踪地图点位���辑器的功能（战斗策略管理、清空等） @mfkvfhpdx 
...
* 联机情况下识别角色队伍时，修复错误识别房主的问题（���导致队伍识别出错） #777
...
* 残血、死亡时自动传送至七天神像
* 恢复界面上单独执行任务的右键菜单
* 添加路线重试机制、卡死脱离3次后放弃当前路径进入重试、卡死脱离添加超时时��
...
* 修复了角色���自动秘境结束战斗时，若朝向正西方则无法自动转向古树的问题。 #608
...
* 优化自动剧情OCR错误时，不点击选项的问��� https://github.com/babalae/better-genshin-impact/issues/422
* 自动邀约支持延时选择选项 #440 
* 自动邀约支持自动点击跳过按钮 #335
...
**此改动过大，未完整进行测试，如果出���部分功能不可用的情况，请提 Issue 告诉我。**
...
为了解决自动拾取的高占用，新增了一个OCR识别引擎，识别速度稍慢但是占有低，并���为默认选项。解决此类卡顿问题：#383
...
## 其他
* 修复重启���图器后素材不重新加载的问题（0.27版本新增的BUG，会导致切换分辨率后报错边界溢出）
* 修改游戏分辨率后自动重启截图器
...
**感谢来自 [HutaoFisher](https://github.com/myHuTao-qwq/HutaoFisher) ��秀抛竿算法与高质量数据集 @myHuTao-qwq**
...
* 0.24 版本 WindowsGraphicsCapture 截图方式的优化作为可配置项，如果有掉帧情况，可以尝试在“启动”页关闭此优���功能
* 重构了部分脏代码 @emako 
...
## 快捷键
- 快捷键新增键鼠���听模式，**支持任意键盘单键、鼠标侧键（XButton1、XButton2）**，不支持组合键，在更好的原神启动后才生效。#6  #45 #123 #228
...
- **新增了自动提交物品的功能**。��于没有找到多个物品的测试场景，所以现在只支持一个物品。
...
## 快速传送
- 更加精准的快速传送，**在存在多个传送点的情况下会选择第一个传送点**，���然消耗的性能也增加了
- 新增按住快捷触发快速传送的方式 #127
- 新增了快速传送的开关快捷键 #229 
...
- WindowsGraphicsCapture截图方式的优化，确保能够获取到最新图��，减少截图失败的报错
