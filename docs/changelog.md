...
* 残血、死亡时自动传送至七天神像
* 恢复界面上单独执行任务的右键菜单
* 添加路线重试机制、卡死��离3次后放弃当前路径进入重试、卡死脱离添加超时时间
...
* TP时，当前地图位置���别失败的情况下，会直接通过国家位置进行大地图定位
* TP失败时加入重试机制
...
* 修复伐木上限配置���法突破2000的问题（荒泷一斗被动可以突破这个值）。 #606
...
> [!IMPORTANT]
> **地图追踪功能**：自动传送到最近的锚点，然后根据小地图开始寻路。系��会根据任务文件的途径点和目标点进行导航。
...
```
此版本新增了一些实验中功能，这些功能后��可能会进行大规模变动。
...
* **WindowsGraphicsCapture 在 Windows 10 version 1903 以��版本中（不含）不再展示（因为系统不支持）** #394
...
**识���思路来自群友 `@益蜘蛛` ，实现方法参考了另一位群友@DR-lin-eng编写的自动音游程序： https://github.com/DR-lin-eng/genshin-auto-music**
