...
- 内嵌 VC++ 运行库 14.44.35208.0，**尝试彻底解决“Unable to load DLL 'onnxruntime'or one of its dependencies: 动态链���库(DLL)初始化例程失败” 的问题**
...
![image](https://github.com/user-attachments/assets/47b54ac1-b524-4f62-a7a0-8e2b83221cf0)
    ...
    * 解决方案：关掉后重试一次，手动运行BetterGl.update.exe（此时打���软件应该已经是最新版了）。如果还是不行，可以直接重新下载安装包安装。
...
- 如果首个点是非TP点位，强制设置在这个点位附近优��做局部匹配，提升匹配速度
...
- 一些UI界面上的变化
- wpfui 更新至 4.0 (��层没啥变化，但是ui框架底层变动)
...
* 新增路径点位 `orientation` 用���控制人物朝向 @Yang-z 
* 路径追踪中自动跳过剧情的配置为强制跳过
...
* 一键进出尘歌壶，增加对多人联机下进入尘歌壶���支持 @FannieMC
...
由于键鼠脚本分享时，不同电脑存在很多影响变量，**测试发现��在大量无法正确重放的情况**。
