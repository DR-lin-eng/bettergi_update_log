...
- **优化自动拾取对聚所的判断**
- **自动拾取添加模糊匹配的名单**
- 自动圣���物转化功能可识别并标记未激活副词条 (#2258) @bhbghghbgb
...
- 取消兑换码使用后不再弹出此剪切板内容的兑换码，新增兑换码剪切板监听的开��配置
...
- 修复配置组名称前半段有同名不显��的问题 (#1858) @kaedelcb
...
- 内嵌 VC++ 运行库 14.44.35208.0，**尝试���底解决“Unable to load DLL 'onnxruntime'or one of its dependencies: 动态链接库(DLL)初始化例程失败” 的问题**
...
## 自动拾取
- **拾取列表已满时候，滑动滚轮（注意这个变更会会导致拾取��成从下往上拾取，对于尘歌壶收菜有一定影响）**
...
- 增加对WaypointForTrack的DisablePickupAround属性支持，可以关闭挖矿动���的周围拾取 @Takaranoao #1322
...
* 自动战斗后、传送前都会释放所有按键，尝试解决TP神像后持续往��走的问题
* 修复火元素采集不生效的问题
* 优化自动传送的稳定性 @physligl
...
**新增了一条龙功能，一键完成日常，更多一条龙功能��划请查看 https://github.com/babalae/better-genshin-impact/issues/846**
...
* 残血、死亡时自动传送至七天神像
* 恢复界面上单独执行任务的右键菜单
* 添加路线重试机制、卡死脱离3次后放弃当前路径进���重试、卡死脱离添加超时时间
...
* TP时，当前地图位置识别失败的情况下，会直���通过国家位置进行大地图定位
* TP失败时加入重试机制
...
> **[minimap](https://github.com/tignioj/minimap) 是一个高精度原神自动采集工具，当然它的功能远不止这些，强烈推��！！**
...
* 新增一种**后台点击模式**，开启时自动剧情和邀约可以在后台被点击。
	* **但是请注意游戏不能最小化，切在后台选择选项时候鼠标仍旧会移动！自动剧情结束后鼠标会被吸附回游戏中，请按<kbd>Alt</kbd>+<kbd>Tab</kbd>键切��到游戏**
...
* 修复凯瑟琳每日自动领取委托/自动重新派遣关闭功���无效的问题 https://github.com/babalae/better-genshin-impact/issues/417
...
如果你平时使用BitBlt截图模式无法正常使用BetterGI，且自动活动音游功能也无法��用，可以尝试此方法： https://github.com/babalae/better-genshin-impact/issues/92
