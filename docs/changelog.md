...
- 修复背包���量 OCR 首尾空白和物品图标后台保存时的对象释放问题 (#3532) @taozhe6
...
### 其他
- 完善截图、图像识别和窗口��阅的资源释放，修复多处内存泄漏 (#3421)
...
- 新增 WinUI 风格导航选中动画并重写 Markdown 渲染组�� (#3369)
```
...
- 添加通过 OCR 识别当��角色 UID 的 JS API (#2988) @Kirito520Asuna
...
- 适配 6.0 菈乌玛退队后队伍UI不会还原，但是跨地图传���后UI还原 的场景
...
- JS脚本中 KeyPress 语法���对功能键也采取直接触发的方式 #2071
...
- 地图追踪版本要求修改为强制
- 优化地图追踪的距离过远判断
- 修复地图追踪中角色公转的场景，持续角度一直未在5度内时，直接强制等到转��到目标位置 #1644
...
- 识别选择队伍后队伍切换成功的场景，��少因为切换时间过长而引发的队伍切换失败的问题 #1457
